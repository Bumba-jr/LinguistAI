...
import {
    generateGermanListening, generateGermanReading,
    GermanListening, GermanReading, GoetheLevel,
    GOETHE_VOCAB_TOPICS,
} from '../../services/germanService';
import { logWeakness, getCachedLesson, cacheLesson } from '../../services/germanStorage';
...
export const LevelBar = ({ level, onLevelChange }: { level: GoetheLevel; onLevelChange: (l: GoetheLevel) => void }) => (
    <div className="flex gap-1.5 flex-wrap">
        {(['A1', 'A2', 'B1', 'B2', 'C1', 'C2'] as GoetheLevel[]).map(l => (
...
export const GermanListeningTrainer = ({ level, onLevelChange, onDone }: {
    level: GoetheLevel; onLevelChange: (l: GoetheLevel) => void;
    onDone: (pct: number, label: string) => void;
}) => {
    const [ex, setEx] = useState<GermanListening | null>(null);
    ...
    const start = async () => {
        ...
        try {
            const e = await generateGermanListening(level);
    ...
    const playAll = () => {
        ...
        const next = () => {
            ...
            speakText(line.de, 'German', () => setTimeout(next, 400), rateRef.current);
        };
        next();
    };
                            ...
                            {ex.lines.map((l, i) => (
                                <div key={i} className="bg-stone-50 rounded-2xl p-3 space-y-0.5">
                                    ...
                                    <div className="flex items-center justify-between gap-2">
                                        <InteractiveText text={l.de} language="German" className="block text-sm font-semibold text-stone-900 flex-1" />
                                        <button onClick={() => speakText(l.de, 'German')} className="text-stone-300 hover:text-indigo-500 shrink-0"><Volume2 size={13} /></button>
                                    </div>
...
export const GermanReadingTrainer = ({ level, onLevelChange, onDone }: {
    ...
    const [ex, setEx] = useState<GermanReading | null>(null);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [answers, setAnswers] = useState<Record<number, string>>({});
    ...
    const start = async () => {
        ...
        try {
            const e = await generateGermanReading(level);
    ...
    return (
        <div className="space-y-5">
            ...
            {!ex && !loading && (
                <div className="bg-white rounded-3xl border border-stone-100 p-6 space-y-3">
                    <p className="font-black text-stone-900">How Goethe reading works</p>
                    <ul className="text-xs text-stone-500 space-y-1.5">
                    ...
                    <div className="bg-white rounded-3xl border border-stone-100 p-6">
                        ...
                        <div className="space-y-3">
                            {ex.paragraphs.map((p, i) => (
                                <div key={i}>
                                    <InteractiveText text={p.de} language="German" className="block text-sm text-stone-800 leading-relaxed" />
...
// ── Vocabulary trainer — Goethe themed domains ─────────────────────────────────
interface VocabWord { term: string; reading: string; en: string; }

export const GermanVocabTrainer = ({ level, onLevelChange }: {
    level: GoetheLevel; onLevelChange: (l: GoetheLevel) => void;
}) => {
    const { addFlashcard, user } = useAppStore() as any;
    const [topicId, setTopicId] = useState(GOETHE_VOCAB_TOPICS[0].id);
    ...
    const topic = GOETHE_VOCAB_TOPICS.find(t => t.id === topicId)!;

    const generate = async (tid = topicId) => {
        const t = GOETHE_VOCAB_TOPICS.find(x => x.id === tid)!;
        ...
        try {
            const r = await generateVocabSet('German', level, t.label, t.hint);
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-5">
                ...
                <p className="text-xs text-stone-400 mb-3">Pick a Goethe domain, get the exam-relevant words, save them to your deck, then quiz yourself.</p>
                <div className="flex gap-1.5 flex-wrap">
                    {GOETHE_VOCAB_TOPICS.map(t => (
                        ...
                        {words.map((w, i) => (
                            <div key={i} className="border border-stone-100 rounded-2xl p-3.5 bg-stone-50/50 flex items-center gap-3">
                                <div className="min-w-0 flex-1">
                                    {tooltips
                                        ? <InteractiveText text={w.term} language="German" className="font-bold text-lg text-stone-900" />
                                ...
                                <button onClick={() => speakText(w.term, 'German')} className="text-stone-300 hover:text-emerald-500 shrink-0"><Volume2 size={14} /></button>
                            ...
                            <div key={i} className="bg-white rounded-3xl border border-stone-100 p-5">
                                ...
                                {dir === 'en2es' ? (
                                    ...
                                    <div className="flex items-center gap-2 mb-3">
                                        {tooltips
                                            ? <InteractiveText text={q.term} language="German" className="text-xl font-black text-stone-900" />
                                            : <p className="text-xl font-black text-stone-900">{q.term}</p>}
                                        <button onClick={() => speakText(q.term, 'German')} className="text-stone-300 hover:text-emerald-500"><Volume2 size={13} /></button>
                                    </div>
                                )}
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    {options.map((opt, oi) => {
...
export const GermanSentenceBuilder = ({ level, onLevelChange }: {
    level: GoetheLevel; onLevelChange: (l: GoetheLevel) => void;
}) => {
    ...
    const generate = async () => {
        ...
        try {
            const r = await generateSentenceTasks('German', level, 5);
    ...
    const check = async () => {
        ...
        try {
            const r = await checkSentenceAttempts('German', level, tasks.map((t, i) => ({
                            ...
                            <div key={i} className="bg-white rounded-3xl border border-stone-100 p-5 space-y-3">
                                <div className="flex items-start justify-between gap-2">
                                    <div className="min-w-0">
                                        ...
                                        <InteractiveText text={t.base} language="German" className="block text-base font-bold text-stone-900" />
                                        <p className="text-xs text-stone-400">{t.baseEnglish}</p>
                                    </div>
                                    <button onClick={() => speakText(t.base, 'German')} className="text-stone-300 hover:text-emerald-500 shrink-0"><Volume2 size={14} /></button>
                                </div>
                                ...
                                {res && (
                                    <div className={cn('rounded-2xl p-3.5 space-y-1 border',
                                        ...
                                        {res.corrected && res.corrected !== '—' && <InteractiveText text={res.corrected} language="German" className="block text-sm font-bold text-stone-900" />}
