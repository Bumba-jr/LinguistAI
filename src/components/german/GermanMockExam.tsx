...
import {
    ...
    goetheWritingTasksFor, goetheSpeakingTasksFor,
    evaluateGermanWriting, evaluateGermanSpeaking, GOETHE_PASS_NOTE,
} from '../../services/germanService';
import { saveMock } from '../../services/germanStorage';
...
const SECTION_LABEL: Record<Phase, string> = {
    intro: '',
    listening: 'Modul Hören — audio plays once',
    reading: 'Modul Lesen — 5 minutes',
    writing: 'Modul Schreiben — 10 minutes',
    speaking: 'Modul Sprechen — 2 minutes',
    report: 'Your simulated Goethe report',
};

// Simulated Goethe score: each test /25, two groups of /50, pass = 30/50 in both.
const to25 = (pct: number) => Math.round((pct / 100) * 25);

export const GermanMockExam = ({ level, onLevelChange }: { level: GoetheLevel; onLevelChange: (l: GoetheLevel) => void }) => {
    const [phase, setPhase] = useState<Phase>('intro');
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [listening, setListening] = useState<GermanListening | null>(null);
    const [reading, setReading] = useState<GermanReading | null>(null);
    ...
    const writingTask = goetheWritingTasksFor(level)[0];
    const speakingTask = goetheSpeakingTasksFor(level)[0];
    ...
    const begin = async () => {
        ...
        try {
            const l = await generateGermanListening(level);
            setListening(l);
            setPhase('listening');
        } catch {
    ...
    const playListening = () => {
        ...
        const next = () => {
            ...
            speakText(line.de, 'German', () => setTimeout(next, 400), 0.88);
        };
        next();
    };
    ...
    const grade = async () => {
        ...
        try {
            if (writingText.trim().split(/\s+/).length >= 20) {
                const fb = await evaluateGermanWriting(writingTask.label, writingTask.guide, writingTask.minWords, writingText, level);
                wPct = Math.round((fb.score25 / 25) * 100); wEst = fb.estimatedLevel;
            }
            if (speakingTranscript.trim()) {
                ...
                const fb = await evaluateGermanSpeaking(speakingTask.label, speakingTask.guide, speakingTask.prompt, speakingTranscript, level);
    ...
    const toggleRecord = () => {
        ...
        const rec = recordAndTranscribe('German' as any, { maxMs: 120000 });
        recRef.current = rec;
            ...
            {phase === 'intro' && (
                <div className="bg-white rounded-3xl border border-stone-100 p-6 space-y-3">
                    <div className="flex items-center gap-2">
                        ...
                        <p className="font-black text-stone-900">Simulated Goethe — {level}</p>
                    </div>
                    <p className="text-xs text-stone-500 leading-relaxed">A compressed full-exam simulation: listening (audio once) → reading (5 min) → writing (10 min) → speaking (2 min). Each test is scored /25 into the two official groups.</p>
                    <div className="bg-amber-50 border border-amber-100 rounded-2xl p-3 text-xs text-amber-800">{GOETHE_PASS_NOTE}</div>
                    ...
                    <div className="bg-white rounded-3xl border border-stone-100 p-6">
                        ...
                        <div className="space-y-3">
                            {reading.paragraphs.map((p, i) => (
                                <InteractiveText key={i} text={p.de} language="German" className="block text-sm text-stone-800 leading-relaxed" />
                            ))}
                        </div>
                    </div>
            ...
            {phase === 'speaking' && (
                <div className="space-y-4">
                    <div className="bg-white rounded-3xl border border-stone-100 p-6">
                        ...
                        <InteractiveText text={speakingTask.prompt} language="German" className="block text-sm font-semibold text-stone-800 mb-1" />
            ...
            {phase === 'report' && report && (
                <div className="space-y-4">
                    <div className={cn('rounded-3xl p-6 text-center text-white', report.passed ? 'bg-emerald-600' : 'bg-stone-900')}>
                        ...
                        <p className="text-4xl font-black my-2">{report.passed ? 'BESTANDEN' : 'NICHT BESTANDEN'}</p>
                        <p className="text-xs opacity-70">Total: {report.total}/100 — pass needs 60 overall (at B1+ most modules also need ≥15/25)</p>
                    </div>
