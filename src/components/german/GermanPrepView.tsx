...
import {
    GoetheLevel, GOETHE_SYLLABUS,
    generateGermanLesson, evaluateGermanWriting, evaluateGermanSpeaking,
    GermanLesson, GermanWritingFeedback, GermanSpeakingFeedback, GOETHE_PASS_NOTE,
    pctToCefr, cefrIndex, goetheWritingTasksFor, goetheSpeakingTasksFor, GOETHE_LEVEL_FORMATS,
} from '../../services/germanService';
import {
    getGoetheScores, addGoetheScore, getCompletedLessons, markLessonComplete,
    GoetheScoreEntry, getWeakLog, getMocks,
    cacheLesson, getCachedLesson,
    getCheckpoints, passCheckpoint,
    getPlan, savePlan, clearPlan,
    getGoetheTarget, setGoetheTarget, setLastLesson, getLastLesson,
} from '../../services/germanStorage';
import { recordAndTranscribe } from '../../services/speechService';
import { LevelBar, GermanListeningTrainer, GermanReadingTrainer, GermanVocabTrainer, GermanSentenceBuilder } from './GermanTrainers';
import { GermanMockExam } from './GermanMockExam';
import { GermanAlphabetChart, GermanFoundations, GermanCheatSheet } from './GermanFoundations';
import { GOETHE_STRATEGY } from '../../services/germanFoundation';
...
// ── shared UI ────────────────────────────────────────────────────────────────
const DeEn = ({ de, en, dark = false }: { de: string; en: string; dark?: boolean }) => (
    <div className="space-y-0.5">
        <InteractiveText text={de} language="German" dark={dark}
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-6">
                ...
                <p className="text-xs text-stone-400 mb-4">Goethe-Zertifikat (Goethe-Institut) — 4 modules (Lesen, Hören, Schreiben, Sprechen), each /25 → /100. Official levels A1 → C2.</p>
                ...
                <div className="mt-4 bg-amber-50 border border-amber-100 rounded-2xl p-3 text-xs text-amber-800">{GOETHE_PASS_NOTE}</div>
            </div>

            {/* Goethe target level — you register for ONE level, so track against it */}
            <div className="bg-white rounded-3xl border border-stone-100 p-6">
                <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                    <div>
                        <h2 className="font-black text-stone-900 flex items-center gap-2"><Target size={16} className="text-emerald-500" /> Your Goethe target level</h2>
                        <p className="text-xs text-stone-400 mt-0.5">Unlike TCF, Goethe grants ONE diploma per level — you register for a specific one.</p>
                    </div>
                    <div className="flex gap-1.5 flex-wrap">
                        {['A1', 'A2', 'B1', 'B2', 'C1', 'C2'].map(t => (
                            <button key={t} onClick={() => setGoetheTarget(t)}
                ...
                <p className="text-[10px] text-stone-300 mt-3 flex items-center gap-1">
                    <AlertTriangle size={10} /> All numbers on this page are practice estimates — not official Goethe results.
                </p>
                {/* what the chosen target level actually looks like, test by test */}
                <div className="mt-4 bg-stone-900 rounded-2xl p-4 text-white">
                    <p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-2">Goethe {target} — what your exam looks like</p>
                    <div className="space-y-1.5 text-xs">
                        {([['Reading', GOETHE_LEVEL_FORMATS[target].reading], ['Listening', GOETHE_LEVEL_FORMATS[target].listening],
                           ['Writing', GOETHE_LEVEL_FORMATS[target].writing], ['Speaking', GOETHE_LEVEL_FORMATS[target].speaking]] as [string, string][]).map(([label, fmt]) => (
...
const Curriculum = ({ language }: { language: string }) => {
    ...
    const [checkpointFor, setCheckpointFor] = useState<GoetheLevel | null>(null);
    ...
    const openLesson = async (topic: { title: string; slug: string; focus: string }) => {
        ...
        const cached = getCachedLesson<GermanLesson>(key);
        if (cached) { setLesson(cached); setLessonKey(key); return; }
        setLoading(true);
        try {
            const l = await generateGermanLesson(level, topic.title, topic.focus, language as any);
            setLesson(l);
            setLessonKey(key);
            cacheLesson(key, l);
        } catch {
    ...
    const saveAllVocab = () => {
        ...
        lesson.vocabulary.forEach(v => {
            ...
            setSavedVocab(prev => new Set(prev).add(v.de));
        });
    };

    const topics = GOETHE_SYLLABUS[level];
    const levelPct = (l: GoetheLevel) => Math.round(GOETHE_SYLLABUS[l].filter(t => done.includes(`${l}:${t.slug}`)).length / GOETHE_SYLLABUS[l].length * 100);
        ...
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-5">
                <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                    ...
                    {last && (
                        <button onClick={() => {
                            setLevel(last.level as GoetheLevel);
                            const topic = GOETHE_SYLLABUS[last.level as GoetheLevel]?.find(t => t.slug === last.slug);
            ...
            {locked && !lesson && (checkpointFor === prevLevel ? (
                <CheckpointQuiz
                    ...
                    topics={GOETHE_SYLLABUS[prevLevel].map(t => `${t.title} (${t.focus})`)}
                    onPass={() => {
            ...
            {lesson && (
                <div className="space-y-5">
                    ...
                    <div className="bg-white rounded-3xl border border-stone-100 p-6">
                        <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-1">{level} · Goethe curriculum</p>
                        <div className="flex items-start justify-between gap-3 flex-wrap">
                            <h1 className="text-2xl font-black text-stone-900 mb-2">{lesson.title}</h1>
                            <LessonFeedbackButton portal="Goethe German" level={level} lessonKey={lessonKey} lessonTitle={lesson.title} />
                        </div>
                                ...
                                <div key={i} className="border border-stone-100 rounded-2xl p-3.5 space-y-1.5 bg-stone-50/50">
                                    <div className="flex items-baseline justify-between gap-2">
                                        <div className="flex items-baseline gap-1.5 flex-wrap">
                                            <InteractiveText text={v.de} language="German" className="font-bold text-stone-900" />
                                            {v.gender && (
                                        ...
                                        <button onClick={() => speakText(v.de, 'German')} className="text-stone-300 hover:text-emerald-500 shrink-0"><Volume2 size={13} /></button>
                                    </div>
                                    <p className="text-sm text-stone-500">{v.en}</p>
                                    {v.example && <DeEn de={v.example.de} en={v.example.en} />}
                            ...
                            <div className="space-y-2">
                                {lesson.pronunciation.map((p, i) => (
                                    <div key={i} className="flex items-center gap-3 bg-stone-50 rounded-xl px-3 py-2">
                                        <span className="font-bold text-stone-800 text-sm">{p.de}</span>
                                        <span className="text-xs font-mono text-violet-500">/{p.approx}/</span>
                                        <span className="text-xs text-stone-400 flex-1">{p.en}</span>
                                        <button onClick={() => speakText(p.de, 'German')} className="text-stone-300 hover:text-emerald-500"><Volume2 size={13} /></button>
                                    </div>
                                ))}
                            </div>
                        </LessonSection>
                            ...
                            <div className="overflow-hidden rounded-2xl border border-stone-100">
                                {lesson.transformations.map((t, i) => (
                                    <div key={i} className={cn('flex items-start gap-3 px-4 py-2.5', i % 2 === 0 ? 'bg-white' : 'bg-stone-50')}>
                                        ...
                                        <div className="flex-1 min-w-0"><DeEn de={t.de} en={t.en} /></div>
                                    </div>
                                ))}
                            </div>
                        </LessonSection>
                    ...
                    <LessonSection title="Practice & translation" icon={<CheckCircle2 size={13} />}>
                            ...
                            {lesson.practice.map((ex, i) => {
                                ...
                                return (
                                    <div key={i} className="bg-stone-50 rounded-2xl p-4">
                                        ...
                                        <InteractiveText text={ex.question} language="German" className="block text-sm font-semibold text-stone-800" />
                                        <div className="flex items-center gap-2 mt-2">
                                            <button onClick={() => setAnswers(prev => ({ ...prev, [`p${i}`]: 'revealed' }))}
                                                className="text-[11px] font-bold text-emerald-600 hover:text-emerald-700">
                                                {picked ? `Answer: ${ex.answer}` : 'Reveal answer'}
                                            </button>
                                            <button onClick={() => speakText(ex.answer, 'German')} className="text-stone-300 hover:text-emerald-500"><Volume2 size={12} /></button>
                                        </div>
                                    </div>
                                );
                            })}
                        ...
                        <div className="space-y-3">
                            {lesson.reverseTranslation?.map((t, i) => {
                                ...
                                return (
                                    <div key={i} className="bg-stone-50 rounded-2xl p-4">
                                        <InteractiveText text={t.de} language="German" className="block text-sm font-semibold text-stone-800 mb-1.5" />
                    ...
                    {/* culture */}
                    {lesson.culture && (
                        <LessonSection title="Culture — the German-speaking world" icon={<Flag size={13} />}>
                    ...
                    <LessonSection title="Mini test" icon={<Trophy size={13} />}>
                        <div className="space-y-4">
                            {lesson.miniTest.map((q, i) => {
                                ...
                                return (
                                    <div key={i}>
                                        <InteractiveText text={q.question} language="German" className="block text-sm font-bold text-stone-800 mb-2" />
...
// ── Writing trainer ──────────────────────────────────────────────────────────
const WritingTrainer = ({ level, onLevelChange }: { level: GoetheLevel; onLevelChange: (l: GoetheLevel) => void }) => {
    // Task set is LEVEL-ACCURATE: A1 writes 15–40-word forms, B2 writes 150–180-word essays with sources…
    const tasks = useMemo(() => goetheWritingTasksFor(level), [level]);
    ...
    const [feedback, setFeedback] = useState<GermanWritingFeedback | null>(null);
    const [error, setError] = useState<string | null>(null);
    ...
    const submit = async () => {
        ...
        try {
            const fb = await evaluateGermanWriting(task.label, task.guide, task.minWords, text, level);
            setFeedback(fb);
            const pct = Math.round((fb.score25 / 25) * 100);
            addGoetheScore({ pct, skill: 'writing', label: `${level} ${task.label}`, score: pct });
        } catch {
...
// ── Speaking trainer ─────────────────────────────────────────────────────────
const SpeakingTrainer = ({ level, language, onLevelChange }: { level: GoetheLevel; language: string; onLevelChange: (l: GoetheLevel) => void }) => {
    // Task set is LEVEL-ACCURATE: A1 has 4 short tasks, B2 adds the graphic monologue, C2 is negotiation…
    const tasks = useMemo(() => goetheSpeakingTasksFor(level), [level]);
    const [taskId, setTaskId] = useState(tasks[0].id);
    const task = tasks.find(t => t.id === taskId) ?? tasks[0];
    ...
    const [feedback, setFeedback] = useState<GermanSpeakingFeedback | null>(null);
    const [error, setError] = useState<string | null>(null);
    ...
    const scoreSpeaking = async (label: string, combined: string) => {
        ...
        addGoetheScore({ pct, skill: 'speaking', label, score: pct });
    };

    const finishLiveExam = async (combined: string) => {
        setEvaluatingLive(true); setError(null); setFeedback(null);
        try {
            const fb = await evaluateGermanSpeaking(task.label, task.guide, 'Interactive examiner session — multiple questions answered live', combined, level);
            setFeedback(fb);
            await scoreSpeaking(`Goethe live ${task.label}`, combined);
        } catch {
    ...
    const toggleRecord = () => {
        ...
        rec.promise.then(async (t) => {
            ...
            setFeedback(null);
            try {
                const fb = await evaluateGermanSpeaking(task.label, task.guide, task.prompt, t, level);
                setFeedback(fb);
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-6">
                ...
                {mode === 'self' && <InteractiveText text={task.prompt} language="German" className="block text-sm font-semibold text-stone-800 mb-1" />}
                <p className="text-xs text-stone-400">{task.guide}</p>
...
// ── Progress tab ─────────────────────────────────────────────────────────────
const Progress = ({ scores }: { scores: GoetheScoreEntry[] }) => {
                ...
                {mocks.slice(0, 5).map((m, i) => (
                    <div key={i} className={cn('flex items-center gap-3 rounded-2xl px-4 py-2.5', i === 0 ? 'bg-stone-900' : 'bg-stone-50')}>
                        ...
                        <span className={cn('text-[9px] font-black px-2 py-0.5 rounded-full', i === 0 ? 'bg-amber-400 text-stone-900' : 'bg-stone-200 text-stone-500')}>
                            {m.passed ? 'BESTANDEN' : `weak: ${m.weakest}`}
                        </span>
                    </div>
                ))}
            </div>
    ...
    return (
        <div className="space-y-4">
            ...
            <p className="text-[10px] text-stone-300 text-center flex items-center justify-center gap-1">
                <AlertTriangle size={10} /> Practice estimates only — not official Goethe results.
            </p>
        </div>
    );
};
...
const GermanPrepView = () => {
    ...
    useEffect(() => { setScores(getGoetheScores()); }, [tab]);

    // German-only exam — functional gate, same as TCF for French.
    if (quizSettings?.targetLanguage !== 'German') {
        return (
            <div className="max-w-md mx-auto w-full py-16 px-6 text-center">
                ...
                <h1 className="text-2xl font-black text-stone-900">Goethe-Zertifikat is German-only</h1>
                <p className="text-stone-400 text-sm mt-2 leading-relaxed">
                    This portal prepares you for the Goethe-Zertifikat (Goethe-Institut), so it stays locked
    ...
    return (
        <div className="max-w-3xl mx-auto w-full py-8 px-4">
            ...
            <div className="flex items-center justify-between mb-5">
                <div>
                    <h1 className="text-2xl font-black text-stone-900 flex items-center gap-2">
                        <GraduationCap size={24} className="text-emerald-600" /> Goethe-Zertifikat
                    </h1>
                    <p className="text-stone-400 text-sm mt-0.5">Exam prep portal — train all four skills, Goethe-Institut style</p>
                </div>
            ...
            {tab === 'vocab' && <GermanVocabTrainer level={level} onLevelChange={setLevel} />}
            {tab === 'builder' && <GermanSentenceBuilder level={level} onLevelChange={setLevel} />}
            {tab === 'foundations' && (
                <div className="space-y-5">
                    <GermanAlphabetChart />
                    <GermanFoundations />
            ...
            {tab === 'cheatsheet' && <GermanCheatSheet />}
            {tab === 'mock' && <GermanMockExam level={level} onLevelChange={setLevel} />}
            {tab === 'listening' && (
                <GermanListeningTrainer level={level} onLevelChange={setLevel} onDone={(pct, label) => {
                    addGoetheScore({ pct, skill: 'listening', label, score: pct });
                }} />
            )}
            {tab === 'reading' && (
                <GermanReadingTrainer level={level} onLevelChange={setLevel} onDone={(pct, label) => {
                    addGoetheScore({ pct, skill: 'reading', label, score: pct });
                }} />
            )}
