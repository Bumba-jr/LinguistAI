...
import { Loader2, Volume2, CheckCircle2, XCircle, RotateCcw, Eye, AlertTriangle, Play, MessageSquareText, MessagesSquare } from 'lucide-react';
...
import { logWeakness, getCachedLesson, cacheLesson } from '../../services/tcfStorage';
...
// ── Vocabulary trainer — TCF Canada themed domains (master curriculum Phase 33) ──
interface VocabWord { term: string; reading: string; en: string; }

export const TCFVocabTrainer = ({ level, onLevelChange }: {
    level: TcfLevel; onLevelChange: (l: TcfLevel) => void;
}) => {
    const { addFlashcard, user } = useAppStore() as any;
    const [topicId, setTopicId] = useState(TCF_VOCAB_TOPICS[0].id);
    const [words, setWords] = useState<VocabWord[] | null>(null);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [saved, setSaved] = useState<Set<string>>(new Set());
    const [tooltips, setTooltips] = useState(true);
    const [quiz, setQuiz] = useState<{ q: VocabWord; dir: 'en2fr' | 'fr2en'; options: VocabWord[] }[] | null>(null);
    const [answers, setAnswers] = useState<Record<number, string>>({});

    const topic = TCF_VOCAB_TOPICS.find(t => t.id === topicId)!;

    const generate = async (tid = topicId) => {
        const t = TCF_VOCAB_TOPICS.find(x => x.id === tid)!;
        ...
        try {
            const r = await generateVocabSet('French', level, t.label, t.hint);
    ...
    const startQuiz = () => {
        ...
        const qs = shuffled.map((w, i) => {
            const dir: 'en2fr' | 'fr2en' = i % 2 === 0 ? 'en2fr' : 'fr2en';
    ...
    const correct = quiz ? quiz.filter(({ q, dir }, i) => answers[i] === (dir === 'en2fr' ? q.term : q.en)).length : 0;
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-5">
                ...
                <p className="text-xs text-stone-400 mb-3">Pick a TCF domain, get the exam-relevant words, save them to your deck, then quiz yourself.</p>
                <div className="flex gap-1.5 flex-wrap">
                    {TCF_VOCAB_TOPICS.map(t => (
                ...
                <div className="flex items-center justify-between gap-3 mt-3 flex-wrap">
                    ...
                    {/* tooltip toggle — tooltips show the click-to-pin word cards */}
            ...
            {words && !quiz && (
                <div className="space-y-3">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                        <p className="text-[11px] text-stone-400">{words.length} words · {tooltips ? 'tap any French word for its word card' : 'tooltips off'} · tap 🔊 to hear it</p>
                        ...
                        {words.map((w, i) => (
                            <div key={i} className="border border-stone-100 rounded-2xl p-3.5 bg-stone-50/50 flex items-center gap-3">
                                <div className="min-w-0 flex-1">
                                    {tooltips
                                        ? <InteractiveText text={w.term} language="French" className="font-bold text-lg text-stone-900" />
                                ...
                                <button onClick={() => speakText(w.term, 'French')} className="text-stone-300 hover:text-emerald-500 shrink-0"><Volume2 size={14} /></button>
            ...
            {quiz && (
                <div className="space-y-3">
                    {quiz.map(({ q, dir, options }, i) => {
                        const target = dir === 'en2fr' ? q.term : q.en;
                        const picked = answers[i];
                        return (
                            <div key={i} className="bg-white rounded-3xl border border-stone-100 p-5">
                                <p className="text-[10px] font-black text-stone-300 uppercase tracking-widest mb-2">{i + 1} · {dir === 'en2fr' ? 'Which one is it?' : 'What does this mean?'}</p>
                                {dir === 'en2fr' ? (
                                    <p className="text-sm font-bold text-stone-800 mb-3">{q.en}</p>
                                ) : (
                                    <div className="flex items-center gap-2 mb-3">
                                        {tooltips
                                            ? <InteractiveText text={q.term} language="French" className="text-xl font-black text-stone-900" />
                                            : <p className="text-xl font-black text-stone-900">{q.term}</p>}
                                        <button onClick={() => speakText(q.term, 'French')} className="text-stone-300 hover:text-emerald-500"><Volume2 size={13} /></button>
                                    </div>
                                )}
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    {options.map((opt, oi) => {
                                        const optValue = dir === 'en2fr' ? opt.term : opt.en;
                                        const revealed = picked !== undefined;
                                        return (
                                            <button key={oi} onClick={() => { if (!revealed) { setAnswers(prev => ({ ...prev, [i]: optValue })); if (optValue !== target) logWeakness({ skill: 'reading', level, question: `Vocab: ${dir === 'en2fr' ? q.en : q.term}`, chosen: optValue, answer: target }); } }}
                                                ...
                                                {dir === 'en2fr' ? <span className="font-bold">{opt.term}</span> : opt.en}
                                            </button>
                                        );
                                    })}
                                </div>
...
// ── Sentence Builder — the transformation & expansion engine (master curriculum Phase 10/25) ──
export const SentenceBuilder = ({ level, onLevelChange }: {
    level: TcfLevel; onLevelChange: (l: TcfLevel) => void;
}) => {
    ...
    const generate = async () => {
        ...
        try {
            const r = await generateSentenceTasks('French', level, 5);
    ...
    const check = async () => {
        ...
        try {
            const r = await checkSentenceAttempts('French', level, tasks.map((t, i) => ({
            ...
            {!tasks && !loading && (
                <div className="bg-white rounded-3xl border border-stone-100 p-6 space-y-3">
                    ...
                    <p className="text-xs text-stone-500 space-y-1.5">
                        One base sentence, five transformations — the most powerful way to learn French: instead of memorising phrases, you learn how the scaffolding changes.
                    </p>
                            ...
                            <div key={i} className="bg-white rounded-3xl border border-stone-100 p-5 space-y-3">
                                <div className="flex items-start justify-between gap-2">
                                    <div className="min-w-0">
                                        ...
                                        <InteractiveText text={t.base} language="French" className="block text-base font-bold text-stone-900" />
                                        <p className="text-xs text-stone-400">{t.baseEnglish}</p>
                                    </div>
                                    <button onClick={() => speakText(t.base, 'French')} className="text-stone-300 hover:text-emerald-500 shrink-0"><Volume2 size={14} /></button>
                                </div>
                                ...
                                {res && (
                                    <div className={cn('rounded-2xl p-3.5 space-y-1 border',
                                        ...
                                        {res.corrected && res.corrected !== '—' && <InteractiveText text={res.corrected} language="French" className="block text-sm font-bold text-stone-900" />}
