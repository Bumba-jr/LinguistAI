...
import {
    ...
    CILSScoreEntry, getWeakLog, getMocks,
    cacheLesson, getCachedLesson,
    getCheckpoints, passCheckpoint,
    getPlan, savePlan, clearPlan,
    getCILSTarget, setCILSTarget, setLastLesson, getLastLesson,
} from '../../services/italianStorage';
import { recordAndTranscribe } from '../../services/speechService';
import { LevelBar, ItalianListeningTrainer, ItalianReadingTrainer, ItalianVocabTrainer, ItalianSentenceBuilder } from './ItalianTrainers';
import { ItalianMockExam } from './ItalianMockExam';
import { ItalianAlphabetChart, ItalianFoundations, ItalianCheatSheet } from './ItalianFoundations';
import { CILS_STRATEGY } from '../../services/italianFoundation';
...
const LEVELS: CilsLevel[] = ['A1', 'A2', 'B1', 'B2', 'C1', 'C2'];
type ItalianTab = 'overview' | 'curriculum' | 'foundations' | 'cheatsheet' | 'vocab' | 'builder' | 'mock' | 'listening' | 'reading' | 'writing' | 'speaking' | 'progress';
...
// ── shared UI ────────────────────────────────────────────────────────────────
const ItEn = ({ it, en, dark = false }: { it: string; en: string; dark?: boolean }) => (
    <div className="space-y-0.5">
        <InteractiveText text={it} language="Italian" dark={dark}
...
// ── Overview tab ─────────────────────────────────────────────────────────────
const Overview = ({ onGo }: { onGo: (t: ItalianTab) => void }) => {
    const scores = useMemo(() => getCILSScores(), []);
    const target = getCILSTarget();
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-6">
                ...
                <p className="text-xs text-stone-400 mb-4">CILS — Certificazione di Italiano come Lingua Straniera (Università per Stranieri di Siena). Skills graded independently; B-levels /100. Official levels A1 → C2.</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {(Object.keys(SKILL_META) as (keyof typeof SKILL_META)[]).map(sk => {
                        const m = SKILL_META[sk];
                        const last = latest(sk);
                        return (
                            <button key={sk} onClick={() => onGo(sk as ItalianTab)}
                ...
                <div className="mt-4 bg-amber-50 border border-amber-100 rounded-2xl p-3 text-xs text-amber-800">{CILS_PASS_NOTE}</div>
            </div>
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-6">
                <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                    <div>
                        <h2 className="font-black text-stone-900 flex items-center gap-2"><Target size={16} className="text-emerald-500" /> Your CILS target level</h2>
                        <p className="text-xs text-stone-400 mt-0.5">Unlike TCF, CILS grants ONE diploma per level — you register for a specific one.</p>
                    </div>
                    <div className="flex gap-1.5 flex-wrap">
                        {['A1', 'A2', 'B1', 'B2', 'C1', 'C2'].map(t => (
                            <button key={t} onClick={() => setCILSTarget(t)}
                ...
                <p className="text-[10px] text-stone-300 mt-3 flex items-center gap-1">
                    <AlertTriangle size={10} /> All numbers on this page are practice estimates — not official CILS results.
                </p>
            ...
            {/* exam plan & countdown */}
            <ExamPlanCard
                examName="CILS-Zertifikat"
                levelLabel={`CILS ${getCILSTarget()} target`}
                language="Italian"
                plan={getPlan()}
                onSavePlan={savePlan}
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-6">
                ...
                <p className="text-xs text-stone-400 mb-3">Knowing the traps is worth as many points as knowing the Italian.</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {CILS_STRATEGY.map(s => (
...
const Curriculum = ({ language }: { language: string }) => {
    ...
    const [level, setLevel] = useState<CilsLevel>('A1');
    const [openTopic, setOpenTopic] = useState<string | null>(null);
    const [lesson, setLesson] = useState<ItalianLesson | null>(null);
    ...
    const [checkpointFor, setCheckpointFor] = useState<CilsLevel | null>(null);
    ...
    const openLesson = async (topic: { title: string; slug: string; focus: string }) => {
        ...
        const cached = getCachedLesson<ItalianLesson>(key);
        if (cached) { setLesson(cached); setLessonKey(key); return; }
        setLoading(true);
        try {
            const l = await generateItalianLesson(level, topic.title, topic.focus, language as any);
            setLesson(l);
            setLessonKey(key);
            cacheLesson(key, l);
        } catch {
    ...
    const saveAllVocab = () => {
        ...
        lesson.vocabulary.forEach(v => {
            ...
            setSavedVocab(prev => new Set(prev).add(v.it));
        });
    };

    const topics = CILS_SYLLABUS[level];
    const levelPct = (l: CilsLevel) => Math.round(CILS_SYLLABUS[l].filter(t => done.includes(`${l}:${t.slug}`)).length / CILS_SYLLABUS[l].length * 100);
        ...
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-5">
                <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                    ...
                    {last && (
                        <button onClick={() => {
                            setLevel(last.level as CilsLevel);
                            const topic = CILS_SYLLABUS[last.level as CilsLevel]?.find(t => t.slug === last.slug);
            ...
            {locked && !lesson && (checkpointFor === prevLevel ? (
                <CheckpointQuiz
                    ...
                    topics={CILS_SYLLABUS[prevLevel].map(t => `${t.title} (${t.focus})`)}
                    onPass={() => {
            ...
            {lesson && (
                <div className="space-y-5">
                    ...
                    <div className="bg-white rounded-3xl border border-stone-100 p-6">
                        <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-1">{level} · CILS curriculum</p>
                        <div className="flex items-start justify-between gap-3 flex-wrap">
                            <h1 className="text-2xl font-black text-stone-900 mb-2">{lesson.title}</h1>
                            <LessonFeedbackButton portal="CILS Italian" level={level} lessonKey={lessonKey} lessonTitle={lesson.title} />
                        </div>
                    ...
                    <LessonSection title={`Vocabulary (${lesson.vocabulary.length} items)`} icon={<BookOpen size={13} />}>
                        <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                            <p className="text-[11px] text-stone-400">Every item: Italian, article, plural, example, related words — tap any word for more.</p>
                                ...
                                <div key={i} className="border border-stone-100 rounded-2xl p-3.5 space-y-1.5 bg-stone-50/50">
                                    <div className="flex items-baseline justify-between gap-2">
                                        <div className="flex items-baseline gap-1.5 flex-wrap">
                                            <InteractiveText text={v.it} language="Italian" className="font-bold text-stone-900" />
                                            {v.gender && (
                                        ...
                                        <button onClick={() => speakText(v.it, 'Italian')} className="text-stone-300 hover:text-emerald-500 shrink-0"><Volume2 size={13} /></button>
                                    </div>
                                    <p className="text-sm text-stone-500">{v.en}</p>
                                    {v.example && <ItEn it={v.example.it} en={v.example.en} />}
                            ...
                            <div className="space-y-2">
                                {lesson.pronunciation.map((p, i) => (
                                    <div key={i} className="flex items-center gap-3 bg-stone-50 rounded-xl px-3 py-2">
                                        <span className="font-bold text-stone-800 text-sm">{p.it}</span>
                                        <span className="text-xs font-mono text-violet-500">/{p.approx}/</span>
                                        <span className="text-xs text-stone-400 flex-1">{p.en}</span>
                                        <button onClick={() => speakText(p.it, 'Italian')} className="text-stone-300 hover:text-emerald-500"><Volume2 size={13} /></button>
                                    </div>
                                ))}
                            </div>
                        </LessonSection>
                            ...
                            <div className="overflow-hidden rounded-2xl border border-stone-100">
                                {lesson.transformations.map((t, i) => (
                                    <div key={i} className={cn('flex items-start gap-3 px-4 py-2.5', i % 2 === 0 ? 'bg-white' : 'bg-stone-50')}>
                                        ...
                                        <div className="flex-1 min-w-0"><ItEn it={t.it} en={t.en} /></div>
                                    </div>
                                ))}
                            </div>
                        </LessonSection>
                    ...
                    <LessonSection title="Practice & translation" icon={<CheckCircle2 size={13} />}>
                            ...
                            {lesson.practice.map((ex, i) => {
                                ...
                                return (
                                    <div key={i} className="bg-stone-50 rounded-2xl p-4">
                                        ...
                                        <InteractiveText text={ex.question} language="Italian" className="block text-sm font-semibold text-stone-800" />
                                        <div className="flex items-center gap-2 mt-2">
                                            <button onClick={() => setAnswers(prev => ({ ...prev, [`p${i}`]: 'revealed' }))}
                                                className="text-[11px] font-bold text-emerald-600 hover:text-emerald-700">
                                                {picked ? `Answer: ${ex.answer}` : 'Reveal answer'}
                                            </button>
                                            <button onClick={() => speakText(ex.answer, 'Italian')} className="text-stone-300 hover:text-emerald-500"><Volume2 size={12} /></button>
                                        </div>
                                    </div>
                                );
                            })}
                                ...
                                return (
                                    <div key={i} className="bg-stone-50 rounded-2xl p-4">
                                        ...
                                        <button onClick={() => setAnswers(prev => ({ ...prev, [`t${i}`]: 'revealed' }))}
                                            className="text-[11px] font-bold text-emerald-600 hover:text-emerald-700">
                                            {picked ? 'Hide' : 'Show the Italian'}
                                        </button>
                                        {picked && <div className="mt-1.5"><ItEn it={t.it} en="" /></div>}
                                    </div>
                                );
                            })}
                        </div>
                        ...
                        <div className="space-y-3">
                            {lesson.reverseTranslation?.map((t, i) => {
                                ...
                                return (
                                    <div key={i} className="bg-stone-50 rounded-2xl p-4">
                                        <InteractiveText text={t.it} language="Italian" className="block text-sm font-semibold text-stone-800 mb-1.5" />
                    ...
                    {/* culture */}
                    {lesson.culture && (
                        <LessonSection title="Culture — Italy and its regions" icon={<Flag size={13} />}>
                    ...
                    <LessonSection title="Mini test" icon={<Trophy size={13} />}>
                        <div className="space-y-4">
                            {lesson.miniTest.map((q, i) => {
                                ...
                                return (
                                    <div key={i}>
                                        <InteractiveText text={q.question} language="Italian" className="block text-sm font-bold text-stone-800 mb-2" />
...
// ── Writing trainer ──────────────────────────────────────────────────────────
const WritingTrainer = ({ level, onLevelChange }: { level: CilsLevel; onLevelChange: (l: CilsLevel) => void }) => {
    // Task set is LEVEL-ACCURATE: A1 writes 15–40-word forms, B2 writes 150–180-word essays with sources…
    const tasks = useMemo(() => cilsWritingTasksFor(level), [level]);
    ...
    const [feedback, setFeedback] = useState<ItalianWritingFeedback | null>(null);
    const [error, setError] = useState<string | null>(null);
    ...
    const submit = async () => {
        ...
        try {
            const fb = await evaluateItalianWriting(task.label, task.guide, task.minWords, text, level);
            setFeedback(fb);
            const pct = Math.round((fb.score100 / 25) * 100);
            addCILSScore({ pct, skill: 'writing', label: `${level} ${task.label}`, score: pct });
        } catch {
...
// ── Speaking trainer ─────────────────────────────────────────────────────────
const SpeakingTrainer = ({ level, language, onLevelChange }: { level: CilsLevel; language: string; onLevelChange: (l: CilsLevel) => void }) => {
    // Task set is LEVEL-ACCURATE: A1 has 4 short tasks, B2 adds the graphic monologue, C2 is negotiation…
    const tasks = useMemo(() => cilsSpeakingTasksFor(level), [level]);
    const [taskId, setTaskId] = useState(tasks[0].id);
    const task = tasks.find(t => t.id === taskId) ?? tasks[0];
    ...
    const [feedback, setFeedback] = useState<ItalianSpeakingFeedback | null>(null);
    const [error, setError] = useState<string | null>(null);
    ...
    const scoreSpeaking = async (label: string, combined: string) => {
        ...
        addCILSScore({ pct, skill: 'speaking', label, score: pct });
    };

    const finishLiveExam = async (combined: string) => {
        setEvaluatingLive(true); setError(null); setFeedback(null);
        try {
            const fb = await evaluateItalianSpeaking(task.label, task.guide, 'Interactive examiner session — multiple questions answered live', combined, level);
            setFeedback(fb);
            await scoreSpeaking(`CILS live ${task.label}`, combined);
        } catch {
    ...
    const toggleRecord = () => {
        ...
        rec.promise.then(async (t) => {
            ...
            setFeedback(null);
            try {
                const fb = await evaluateItalianSpeaking(task.label, task.guide, task.prompt, t, level);
                setFeedback(fb);
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-6">
                ...
                {mode === 'self' && <InteractiveText text={task.prompt} language="Italian" className="block text-sm font-semibold text-stone-800 mb-1" />}
                <p className="text-xs text-stone-400">{task.guide}</p>
...
// ── Progress tab ─────────────────────────────────────────────────────────────
const Progress = ({ scores }: { scores: CILSScoreEntry[] }) => {
    ...
    return (
        <div className="space-y-4">
            ...
            <p className="text-[10px] text-stone-300 text-center flex items-center justify-center gap-1">
                <AlertTriangle size={10} /> Practice estimates only — not official CILS results.
            </p>
        </div>
    );
};
...
// ── Portal shell ─────────────────────────────────────────────────────────────
const ItalianPrepView = () => {
    const { quizSettings, setActiveTab } = useAppStore() as any;
    const language = quizSettings?.targetLanguage || 'Italian';
    const [tab, setTab] = useState<ItalianTab>('overview');
    const [level, setLevel] = useState<CilsLevel>('A2');
    const [scores, setScores] = useState<CILSScoreEntry[]>(getCILSScores());

    useEffect(() => { setScores(getCILSScores()); }, [tab]);

    // Italian-only exam — functional gate, same as TCF for French.
    if (quizSettings?.targetLanguage !== 'Italian') {
        return (
            <div className="max-w-md mx-auto w-full py-16 px-6 text-center">
                ...
                <h1 className="text-2xl font-black text-stone-900">CILS is Italian-only</h1>
                <p className="text-stone-400 text-sm mt-2 leading-relaxed">
                    This portal prepares you for the CILS (Università per Stranieri di Siena), so it stays locked
    ...
    const TABS: { id: ItalianTab; label: string; icon: any }[] = [
    ...
    return (
        <div className="max-w-3xl mx-auto w-full py-8 px-4">
            ...
            <div className="flex items-center justify-between mb-5">
                <div>
                    <h1 className="text-2xl font-black text-stone-900 flex items-center gap-2">
                        <GraduationCap size={24} className="text-emerald-600" /> CILS Italiano
                    </h1>
                    <p className="text-stone-400 text-sm mt-0.5">Exam prep portal — train all four skills, CILS style</p>
                </div>
            ...
            {tab === 'vocab' && <ItalianVocabTrainer level={level} onLevelChange={setLevel} />}
            {tab === 'builder' && <ItalianSentenceBuilder level={level} onLevelChange={setLevel} />}
            {tab === 'foundations' && (
                <div className="space-y-5">
                    <ItalianAlphabetChart />
                    <ItalianFoundations />
                </div>
            )}
            {tab === 'cheatsheet' && <ItalianCheatSheet />}
            {tab === 'mock' && <ItalianMockExam level={level} onLevelChange={setLevel} />}
            {tab === 'listening' && (
                <ItalianListeningTrainer level={level} onLevelChange={setLevel} onDone={(pct, label) => {
                    addCILSScore({ pct, skill: 'listening', label, score: pct });
                }} />
            )}
            {tab === 'reading' && (
                <ItalianReadingTrainer level={level} onLevelChange={setLevel} onDone={(pct, label) => {
                    addCILSScore({ pct, skill: 'reading', label, score: pct });
                }} />
            )}
...
export default ItalianPrepView;
