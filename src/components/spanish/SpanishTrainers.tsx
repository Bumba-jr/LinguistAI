...
import { logWeakness, getCachedLesson, cacheLesson } from '../../services/spanishStorage';
...
export const LevelBar = ({ level, onLevelChange }: { level: DeleLevel; onLevelChange: (l: DeleLevel) => void }) => (
    <div className="flex gap-1.5 flex-wrap">
        {(['A1', 'A2', 'B1', 'B2', 'C1', 'C2'] as DeleLevel[]).map(l => (
...
export const SpanishListeningTrainer = ({ level, onLevelChange, onDone }: {
    level: DeleLevel; onLevelChange: (l: DeleLevel) => void;
    onDone: (pct: number, label: string) => void;
}) => {
    const [ex, setEx] = useState<SpanishListening | null>(null);
    ...
    const start = async () => {
        ...
        try {
            const e = await generateSpanishListening(level);
    ...
    const playAll = () => {
        ...
        const next = () => {
            ...
            speakText(line.es, 'Spanish', () => setTimeout(next, 400), rateRef.current);
        };
        next();
    };
        ...
        <div className="space-y-5">
            ...
            {!ex && !loading && (
                <div className="bg-white rounded-3xl border border-stone-100 p-6 space-y-3">
                    <p className="font-black text-stone-900">How DELE listening works</p>
                    <ul className="text-xs text-stone-500 space-y-1.5">
                        <li>• Real exam: 30 items across 4-5 tasks, each recording plays <b>once</b>.</li>
                        ...
                        <li>• Switch accents early: Spain pronounces c/z as "th"; Latin America says "s".</li>
                    </ul>
                            ...
                            {ex.lines.map((l, i) => (
                                <div key={i} className="bg-stone-50 rounded-2xl p-3 space-y-0.5">
                                    ...
                                    <div className="flex items-center justify-between gap-2">
                                        <InteractiveText text={l.es} language="Spanish" className="block text-sm font-semibold text-stone-900 flex-1" />
                                        <button onClick={() => speakText(l.es, 'Spanish')} className="text-stone-300 hover:text-indigo-500 shrink-0"><Volume2 size={13} /></button>
                                    </div>
...
export const SpanishReadingTrainer = ({ level, onLevelChange, onDone }: {
    level: DeleLevel; onLevelChange: (l: DeleLevel) => void;
    onDone: (pct: number, label: string) => void;
}) => {
    const [ex, setEx] = useState<SpanishReading | null>(null);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [answers, setAnswers] = useState<Record<number, string>>({});
    ...
    const start = async () => {
        ...
        try {
            const e = await generateSpanishReading(level);
    ...
    return (
        <div className="space-y-5">
            ...
            {!ex && !loading && (
                <div className="bg-white rounded-3xl border border-stone-100 p-6 space-y-3">
                    <p className="font-black text-stone-900">How DELE reading works</p>
                    <ul className="text-xs text-stone-500 space-y-1.5">
                        <li>• Real exam: 25-40 items in 45-70 minutes — skim, locate, don't translate everything.</li>
                    ...
                    <div className="bg-white rounded-3xl border border-stone-100 p-6">
                        ...
                        <div className="space-y-3">
                            {ex.paragraphs.map((p, i) => (
                                <div key={i}>
                                    <InteractiveText text={p.es} language="Spanish" className="block text-sm text-stone-800 leading-relaxed" />
...
// ── Vocabulary trainer — DELE themed domains ─────────────────────────────────
interface VocabWord { term: string; reading: string; en: string; }

export const SpanishVocabTrainer = ({ level, onLevelChange }: {
    level: DeleLevel; onLevelChange: (l: DeleLevel) => void;
}) => {
    const { addFlashcard, user } = useAppStore() as any;
    const [topicId, setTopicId] = useState(DELE_VOCAB_TOPICS[0].id);
    ...
    const topic = DELE_VOCAB_TOPICS.find(t => t.id === topicId)!;

    const generate = async (tid = topicId) => {
        const t = DELE_VOCAB_TOPICS.find(x => x.id === tid)!;
        ...
        try {
            const r = await generateVocabSet('Spanish', level, t.label, t.hint);
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-5">
                ...
                <p className="text-xs text-stone-400 mb-3">Pick a DELE domain, get the exam-relevant words, save them to your deck, then quiz yourself.</p>
                <div className="flex gap-1.5 flex-wrap">
                    {DELE_VOCAB_TOPICS.map(t => (
                        ...
                        {words.map((w, i) => (
                            <div key={i} className="border border-stone-100 rounded-2xl p-3.5 bg-stone-50/50 flex items-center gap-3">
                                <div className="min-w-0 flex-1">
                                    {tooltips
                                        ? <InteractiveText text={w.term} language="Spanish" className="font-bold text-lg text-stone-900" />
                                ...
                                <button onClick={() => speakText(w.term, 'Spanish')} className="text-stone-300 hover:text-emerald-500 shrink-0"><Volume2 size={14} /></button>
                            ...
                            <div key={i} className="bg-white rounded-3xl border border-stone-100 p-5">
                                ...
                                {dir === 'en2es' ? (
                                    ...
                                    <div className="flex items-center gap-2 mb-3">
                                        {tooltips
                                            ? <InteractiveText text={q.term} language="Spanish" className="text-xl font-black text-stone-900" />
                                            : <p className="text-xl font-black text-stone-900">{q.term}</p>}
                                        <button onClick={() => speakText(q.term, 'Spanish')} className="text-stone-300 hover:text-emerald-500"><Volume2 size={13} /></button>
                                    </div>
                                )}
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    {options.map((opt, oi) => {
...
export const SpanishSentenceBuilder = ({ level, onLevelChange }: {
    level: DeleLevel; onLevelChange: (l: DeleLevel) => void;
}) => {
    ...
    const generate = async () => {
        ...
        try {
            const r = await generateSentenceTasks('Spanish', level, 5);
    ...
    const check = async () => {
        ...
        try {
            const r = await checkSentenceAttempts('Spanish', level, tasks.map((t, i) => ({
                            ...
                            <div key={i} className="bg-white rounded-3xl border border-stone-100 p-5 space-y-3">
                                <div className="flex items-start justify-between gap-2">
                                    <div className="min-w-0">
                                        ...
                                        <InteractiveText text={t.base} language="Spanish" className="block text-base font-bold text-stone-900" />
                                        <p className="text-xs text-stone-400">{t.baseEnglish}</p>
                                    </div>
                                    <button onClick={() => speakText(t.base, 'Spanish')} className="text-stone-300 hover:text-emerald-500 shrink-0"><Volume2 size={14} /></button>
                                </div>
                                <textarea value={attempts[i] || ''} onChange={e => setAttempts(prev => ({ ...prev, [i]: e.target.value }))} rows={2} disabled={!!results}
                                    placeholder="Escribe tu frase transformada…"
                                ...
                                {res && (
                                    <div className={cn('rounded-2xl p-3.5 space-y-1 border',
                                        ...
                                        {res.corrected && res.corrected !== '—' && <InteractiveText text={res.corrected} language="Spanish" className="block text-sm font-bold text-stone-900" />}
