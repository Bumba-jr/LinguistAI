...
const SECTION_LABEL: Record<Phase, string> = {
    intro: '',
    listening: 'Prueba 2 · Comprensión auditiva — audio plays once',
    reading: 'Prueba 1 · Comprensión de lectura — 5 minutes',
    writing: 'Prueba 3 · Expresión escrita — 10 minutes',
    speaking: 'Prueba 4 · Expresión oral — 2 minutes',
    report: 'Your simulated DELE report',
};

// Simulated DELE score: each test /25, two groups of /50, pass = 30/50 in both.
const to25 = (pct: number) => Math.round((pct / 100) * 25);

export const SpanishMockExam = ({ level, onLevelChange }: { level: DeleLevel; onLevelChange: (l: DeleLevel) => void }) => {
    const [phase, setPhase] = useState<Phase>('intro');
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [listening, setListening] = useState<SpanishListening | null>(null);
    ...
    const [report, setReport] = useState<null | {
        ...
        groupA: number; groupB: number; passed: boolean; weakest: string;
    }>(null);
    ...
    const begin = async () => {
        ...
        try {
            const l = await generateSpanishListening(level);
            setListening(l);
            setPhase('listening');
        } catch {
    ...
    const playListening = () => {
        ...
        const next = () => {
            ...
            speakText(line.es, 'Spanish', () => setTimeout(next, 400), 0.88);
        };
        next();
    };
    ...
    const grade = async () => {
        ...
        try {
            if (writingText.trim().split(/\s+/).length >= 20) {
                const fb = await evaluateSpanishWriting(writingTask.label, writingTask.guide, writingTask.minWords, writingText, level);
            ...
            if (speakingTranscript.trim()) {
                ...
                const fb = await evaluateSpanishSpeaking(speakingTask.label, speakingTask.guide, speakingTask.prompt, speakingTranscript, level);
                sEst = fb.estimatedLevel;
            }
        } catch { /* estimates stay 0 for the failed skill */ }
        const groupA = to25(lPct) + to25(rPct);
        const groupB = to25(wPct) + to25(sPct);
        const passed = groupA >= 30 && groupB >= 30;
    ...
    const toggleRecord = () => {
        ...
        const rec = recordAndTranscribe('Spanish' as any, { maxMs: 120000 });
        recRef.current = rec;
            ...
            {phase === 'intro' && (
                <div className="bg-white rounded-3xl border border-stone-100 p-6 space-y-3">
                    <div className="flex items-center gap-2">
                        ...
                        <p className="font-black text-stone-900">Simulated DELE — {level}</p>
                    </div>
                    ...
                    <div className="bg-amber-50 border border-amber-100 rounded-2xl p-3 text-xs text-amber-800">{DELE_PASS_NOTE}</div>
                    ...
                    <div className="bg-white rounded-3xl border border-stone-100 p-6">
                        ...
                        <div className="space-y-3">
                            {reading.paragraphs.map((p, i) => (
                                <InteractiveText key={i} text={p.es} language="Spanish" className="block text-sm text-stone-800 leading-relaxed" />
                            ))}
                        </div>
                    </div>
            ...
            {phase === 'writing' && (
                <div className="space-y-4">
                    ...
                    <textarea value={writingText} onChange={e => setWritingText(e.target.value)} rows={10}
                        placeholder="Escribe tu respuesta en español… (respeta las tildes y los ¿¡!)"
            ...
            {phase === 'speaking' && (
                <div className="space-y-4">
                    <div className="bg-white rounded-3xl border border-stone-100 p-6">
                        ...
                        <InteractiveText text={speakingTask.prompt} language="Spanish" className="block text-sm font-semibold text-stone-800 mb-1" />
            ...
            {phase === 'report' && report && (
                <div className="space-y-4">
                    <div className={cn('rounded-3xl p-6 text-center text-white', report.passed ? 'bg-emerald-600' : 'bg-stone-900')}>
                        ...
                        <p className="text-4xl font-black my-2">{report.passed ? 'APTO' : 'NO APTO'}</p>
                        <p className="text-xs opacity-70">Grupo A (reading + listening): {report.groupA}/50 · Grupo B (writing + speaking): {report.groupB}/50 — pass needs 30 in both</p>
                    </div>
