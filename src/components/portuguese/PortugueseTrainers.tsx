...
import {
    generatePortugueseListening, generatePortugueseReading,
    PortugueseListening, PortugueseReading, CapleLevel,
    CAPLE_VOCAB_TOPICS,
} from '../../services/portugueseService';
import { logWeakness, getCachedLesson, cacheLesson } from '../../services/portugueseStorage';
...
export const LevelBar = ({ level, onLevelChange }: { level: CapleLevel; onLevelChange: (l: CapleLevel) => void }) => (
...
export const PortugueseListeningTrainer = ({ level, onLevelChange, onDone }: {
    level: CapleLevel; onLevelChange: (l: CapleLevel) => void;
    onDone: (pct: number, label: string) => void;
}) => {
    const [ex, setEx] = useState<PortugueseListening | null>(null);
    ...
    // Listening progression: slow learner Japanese → exam speed → challenge speed
    ...
    const start = async () => {
        ...
        try {
            const e = await generatePortugueseListening(level);
    ...
    const playAll = () => {
        ...
        const next = () => {
            ...
            speakText(line.pt, 'Portuguese', () => setTimeout(next, 400), rateRef.current);
        };
        next();
    };
        ...
        <div className="space-y-5">
            ...
            {!ex && !loading && (
                <div className="bg-white rounded-3xl border border-stone-100 p-6 space-y-3">
                    <p className="font-black text-stone-900">How CAPLE listening works</p>
                    <ul className="text-xs text-stone-500 space-y-1.5">
                        ...
                        <li>• Here: the recording is read aloud in Japanese — try to answer after one play.</li>
                            ...
                            {ex.lines.map((l, i) => (
                                <div key={i} className="bg-stone-50 rounded-2xl p-3 space-y-0.5">
                                    ...
                                    <div className="flex items-center justify-between gap-2">
                                        <InteractiveText text={l.pt} language="Portuguese" className="block text-sm font-semibold text-stone-900 flex-1" />
                                        <button onClick={() => speakText(l.pt, 'Portuguese')} className="text-stone-300 hover:text-indigo-500 shrink-0"><Volume2 size={13} /></button>
                                    </div>
...
export const PortugueseReadingTrainer = ({ level, onLevelChange, onDone }: {
    level: CapleLevel; onLevelChange: (l: CapleLevel) => void;
    onDone: (pct: number, label: string) => void;
}) => {
    const [ex, setEx] = useState<PortugueseReading | null>(null);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [answers, setAnswers] = useState<Record<number, string>>({});
    ...
    const start = async () => {
        ...
        try {
            const e = await generatePortugueseReading(level);
    ...
    return (
        <div className="space-y-5">
            ...
            {!ex && !loading && (
                <div className="bg-white rounded-3xl border border-stone-100 p-6 space-y-3">
                    <p className="font-black text-stone-900">How CAPLE reading works</p>
                    <ul className="text-xs text-stone-500 space-y-1.5">
                    ...
                    <div className="bg-white rounded-3xl border border-stone-100 p-6">
                        ...
                        <div className="space-y-3">
                            {ex.paragraphs.map((p, i) => (
                                <div key={i}>
                                    <InteractiveText text={p.pt} language="Portuguese" className="block text-sm text-stone-800 leading-relaxed" />
...
export const PortugueseVocabTrainer = ({ level, onLevelChange }: {
    level: CapleLevel; onLevelChange: (l: CapleLevel) => void;
}) => {
    const { addFlashcard, user } = useAppStore() as any;
    const [topicId, setTopicId] = useState(CAPLE_VOCAB_TOPICS[0].id);
    ...
    const topic = CAPLE_VOCAB_TOPICS.find(t => t.id === topicId)!;

    const generate = async (tid = topicId) => {
        const t = CAPLE_VOCAB_TOPICS.find(x => x.id === tid)!;
        ...
        try {
            const r = await generateVocabSet('Portuguese', level, t.label, t.hint);
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-5">
                ...
                <p className="text-xs text-stone-400 mb-3">Pick a CAPLE domain, get the exam-relevant words, save them to your deck, then quiz yourself.</p>
                <div className="flex gap-1.5 flex-wrap">
                    {CAPLE_VOCAB_TOPICS.map(t => (
            ...
            {words && !quiz && (
                <div className="space-y-3">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                        <p className="text-[11px] text-stone-400">{words.length} words · {tooltips ? 'tap any Japanese word for its word card' : 'tooltips off'} · tap 🔊 to hear it</p>
                        ...
                        {words.map((w, i) => (
                            <div key={i} className="border border-stone-100 rounded-2xl p-3.5 bg-stone-50/50 flex items-center gap-3">
                                <div className="min-w-0 flex-1">
                                    {tooltips
                                        ? <InteractiveText text={w.term} language="Portuguese" className="font-bold text-lg text-stone-900" />
                                ...
                                <button onClick={() => speakText(w.term, 'Portuguese')} className="text-stone-300 hover:text-emerald-500 shrink-0"><Volume2 size={14} /></button>
                            ...
                            <div key={i} className="bg-white rounded-3xl border border-stone-100 p-5">
                                ...
                                {dir === 'en2es' ? (
                                    ...
                                    <div className="flex items-center gap-2 mb-3">
                                        {tooltips
                                            ? <InteractiveText text={q.term} language="Portuguese" className="text-xl font-black text-stone-900" />
                                            : <p className="text-xl font-black text-stone-900">{q.term}</p>}
                                        <button onClick={() => speakText(q.term, 'Portuguese')} className="text-stone-300 hover:text-emerald-500"><Volume2 size={13} /></button>
                                    </div>
                                )}
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    {options.map((opt, oi) => {
...
export const PortugueseSentenceBuilder = ({ level, onLevelChange }: {
    level: CapleLevel; onLevelChange: (l: CapleLevel) => void;
}) => {
    ...
    const generate = async () => {
        ...
        try {
            const r = await generateSentenceTasks('Portuguese', level, 5);
    ...
    const check = async () => {
        ...
        try {
            const r = await checkSentenceAttempts('Portuguese', level, tasks.map((t, i) => ({
            ...
            {!tasks && !loading && (
                <div className="bg-white rounded-3xl border border-stone-100 p-6 space-y-3">
                    ...
                    <p className="text-xs text-stone-500 space-y-1.5">
                        One base sentence, five transformations — the most powerful way to learn Japanese: instead of memorising phrases, you learn how the scaffolding changes (particles, verb forms, register, word order).
                    </p>
                            ...
                            <div key={i} className="bg-white rounded-3xl border border-stone-100 p-5 space-y-3">
                                <div className="flex items-start justify-between gap-2">
                                    <div className="min-w-0">
                                        ...
                                        <InteractiveText text={t.base} language="Portuguese" className="block text-base font-bold text-stone-900" />
                                        <p className="text-xs text-stone-400">{t.baseEnglish}</p>
                                    </div>
                                    <button onClick={() => speakText(t.base, 'Portuguese')} className="text-stone-300 hover:text-emerald-500 shrink-0"><Volume2 size={14} /></button>
                                </div>
                                ...
                                {res && (
                                    <div className={cn('rounded-2xl p-3.5 space-y-1 border',
                                        ...
                                        {res.corrected && res.corrected !== '—' && <InteractiveText text={res.corrected} language="Portuguese" className="block text-sm font-bold text-stone-900" />}
