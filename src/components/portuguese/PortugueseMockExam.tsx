...
import {
    ...
    capleWritingTasksFor, capleSpeakingTasksFor,
    evaluatePortugueseWriting, evaluatePortugueseSpeaking, CAPLE_PASS_NOTE, CAPLE_OF_LEVEL,
} from '../../services/portugueseService';
import { saveMock } from '../../services/portugueseStorage';
...
const SECTION_LABEL: Record<Phase, string> = {
    intro: '',
    listening: 'Compreensão do Oral — audio plays once',
    reading: 'Compreensão da Leitura — 5 minutes',
    writing: 'Produção Escrita — 10 minutes',
    speaking: 'Produção Oral — 2 minutes',
    report: 'Your simulated CAPLE report',
};

// Simulated CAPLE result: each skill gets a practice %; every skill must reach the pass threshold independently.
const to25 = (pct: number) => Math.round((pct / 100) * 25);

export const PortugueseMockExam = ({ level, onLevelChange }: { level: CapleLevel; onLevelChange: (l: CapleLevel) => void }) => {
    const [phase, setPhase] = useState<Phase>('intro');
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [listening, setListening] = useState<PortugueseListening | null>(null);
    const [reading, setReading] = useState<PortugueseReading | null>(null);
    ...
    const writingTask = capleWritingTasksFor(level)[0];
    const speakingTask = capleSpeakingTasksFor(level)[0];
    ...
    const begin = async () => {
        ...
        try {
            const l = await generatePortugueseListening(level);
            setListening(l);
            setPhase('listening');
        } catch {
    ...
    const playListening = () => {
        ...
        const next = () => {
            ...
            speakText(line.pt, 'Italian', () => setTimeout(next, 400), 0.88);
        };
        next();
    };
    ...
    const finishListening = () => {
        stopSpeaking();
        setLoading(true);
        generatePortugueseReading(level)
    ...
    const grade = async () => {
        ...
        try {
            if (writingText.trim().split(/\s+/).length >= 20) {
                const fb = await evaluatePortugueseWriting(writingTask.label, writingTask.guide, writingTask.minWords, writingText, level);
            ...
            if (speakingTranscript.trim()) {
                ...
                const fb = await evaluatePortugueseSpeaking(speakingTask.label, speakingTask.guide, speakingTask.prompt, speakingTranscript, level);
        ...
        const passed = [rPct, lPct, wPct, sPct].every(pct => pct >= 55);
        const weakest = ([['reading', rPct], ['listening', lPct], ['writing', wPct], ['speaking', sPct]] as [string, number][])
            .sort((a, b) => a[1] - b[1])[0][0];
        const rep = {
            listening: { pct: lPct, ok: lPct >= 55 },
            reading: { pct: rPct, ok: rPct >= 55 },
            writing: { pct: wPct, ok: wPct >= 55, est: wEst },
            speaking: { pct: sPct, ok: sPct >= 55, est: sEst },
            passed, weakest,
        };
        setReport(rep);
    ...
    const toggleRecord = () => {
        ...
        const rec = recordAndTranscribe('Portuguese' as any, { maxMs: 120000 });
        recRef.current = rec;
            ...
            {phase === 'intro' && (
                <div className="bg-white rounded-3xl border border-stone-100 p-6 space-y-3">
                    <div className="flex items-center gap-2">
                        ...
                        <p className="font-black text-stone-900">Simulated CAPLE — {level}</p>
                    </div>
                    <p className="text-xs text-stone-500 leading-relaxed">A compressed full-exam simulation: listening (audio once) → reading (5 min) → writing (10 min) → speaking (2 min). CAPLE grades each skill independently — every skill must reach its own threshold.</p>
                    <div className="bg-amber-50 border border-amber-100 rounded-2xl p-3 text-xs text-amber-800">{CAPLE_PASS_NOTE}</div>
                    ...
                    <div className="bg-white rounded-3xl border border-stone-100 p-6">
                        ...
                        <div className="space-y-3">
                            {reading.paragraphs.map((p, i) => (
                                <InteractiveText key={i} text={p.pt} language="Portuguese" className="block text-sm text-stone-800 leading-relaxed" />
                            ))}
                        </div>
                    </div>
            ...
            {phase === 'speaking' && (
                <div className="space-y-4">
                    <div className="bg-white rounded-3xl border border-stone-100 p-6">
                        ...
                        <InteractiveText text={speakingTask.prompt} language="Portuguese" className="block text-sm font-semibold text-stone-800 mb-1" />
            ...
            {phase === 'report' && report && (
                <div className="space-y-4">
                    <div className={cn('rounded-3xl p-6 text-center text-white', report.passed ? 'bg-emerald-600' : 'bg-stone-900')}>
                        <p className="text-[10px] font-black uppercase tracking-widest opacity-60">Simulated result — {CAPLE_OF_LEVEL[level]}</p>
                        <p className="text-4xl font-black my-2">{report.passed ? 'APROVADO' : 'NÃO APROVADO'}</p>
                        <p className="text-xs opacity-70">Each skill is judged independently — CAPLE lets you retake only the skills you failed</p>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        {([['Reading', report.reading], ['Listening', report.listening], ['Writing', report.writing], ['Speaking', report.speaking]] as [string, { pct: number; ok: boolean; est?: string }][]).map(([label, r]) => (
                            <div key={label} className={cn('rounded-3xl border p-4 text-center', r.ok ? 'bg-emerald-50 border-emerald-100' : 'bg-red-50 border-red-100')}>
                                ...
                                <p className="text-[10px] font-black uppercase tracking-wider">{r.ok ? '✓ Suficiente' : '✗ Insuficiente'}{r.est ? ` · est. ${r.est}` : ''}</p>
                            </div>
                        ))}
                    </div>
