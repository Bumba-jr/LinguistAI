...
import {
    CapleLevel, CAPLE_SYLLABUS,
    generatePortugueseLesson, evaluatePortugueseWriting, evaluatePortugueseSpeaking,
    PortugueseLesson, PortugueseWritingFeedback, PortugueseSpeakingFeedback, CAPLE_PASS_NOTE,
    pctToCefr, cefrIndex, capleWritingTasksFor, capleSpeakingTasksFor, CAPLE_LEVEL_FORMATS, CAPLE_OF_LEVEL,
} from '../../services/portugueseService';
import {
    getCAPLEScores, addCAPLEScore, getCompletedLessons, markLessonComplete,
    CAPLEScoreEntry, getWeakLog, getMocks,
    cacheLesson, getCachedLesson,
    getCheckpoints, passCheckpoint,
    getPlan, savePlan, clearPlan,
    getCAPLETarget, setCAPLETarget, setLastLesson, getLastLesson,
} from '../../services/portugueseStorage';
import { recordAndTranscribe } from '../../services/speechService';
import { LevelBar, PortugueseListeningTrainer, PortugueseReadingTrainer, PortugueseVocabTrainer, PortugueseSentenceBuilder } from './PortugueseTrainers';
import { PortugueseMockExam } from './PortugueseMockExam';
import { PortugueseAccentsChart, PortugueseFoundations, PortugueseCheatSheet } from './PortugueseFoundations';
import { CAPLE_STRATEGY } from '../../services/portugueseFoundation';
...
const LEVELS: CapleLevel[] = ['A1', 'A2', 'B1', 'B2', 'C1', 'C2'];
...
const SKILL_META = {
    listening: { label: 'Compreensão do Oral', icon: Headphones, color: 'text-indigo-500', bg: 'bg-indigo-50', exam: 'CAPLE component · 30-40 min · audio once' },
    reading: { label: 'Compreensão da Leitura', icon: BookOpenCheck, color: 'text-teal-500', bg: 'bg-teal-50', exam: 'CAPLE component · 30-120 min by level' },
    writing: { label: 'Produção Escrita', icon: PenLine, color: 'text-amber-500', bg: 'bg-amber-50', exam: 'CAPLE component · letters, narratives, argumentation' },
    speaking: { label: 'Produção Oral', icon: Mic, color: 'text-rose-500', bg: 'bg-rose-50', exam: 'CAPLE component · 15-25 min · interaction and negotiation' },
} as const;

// ── shared UI ────────────────────────────────────────────────────────────────
const PtEn = ({ pt, en, dark = false }: { pt: string; en: string; dark?: boolean }) => (
    <div className="space-y-0.5">
        <InteractiveText text={pt} language="Portuguese" dark={dark}
...
// ── Overview tab ─────────────────────────────────────────────────────────────
const Overview = ({ onGo }: { onGo: (t: JapaneseTab) => void }) => {
    const scores = useMemo(() => getCAPLEScores(), []);
    const target = getCAPLETarget();
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-6">
                ...
                <p className="text-xs text-stone-400 mb-4">CAPLE — Centro de Avaliação de Português Língua Estrangeira (Universidade de Lisboa). Four skills from B1; scored 55–100% (Suficiente/Bom/Muito Bom). European Portuguese is the target.</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {(Object.keys(SKILL_META) as (keyof typeof SKILL_META)[]).map(sk => {
                        const m = SKILL_META[sk];
                        const last = latest(sk);
                        return (
                            <button key={sk} onClick={() => onGo(sk as JapaneseTab)}
                ...
                <div className="mt-4 bg-amber-50 border border-amber-100 rounded-2xl p-3 text-xs text-amber-800">{CAPLE_PASS_NOTE}</div>
            </div>
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-6">
                <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                    <div>
                        <h2 className="font-black text-stone-900 flex items-center gap-2"><Target size={16} className="text-emerald-500" /> Your CAPLE exam level</h2>
                        <p className="text-xs text-stone-400 mt-0.5">Pick your target — CAPLE has six named exams and you can register directly for any of them without passing lower ones first.</p>
                    </div>
                    <div className="flex gap-1.5 flex-wrap">
                        {['A1', 'A2', 'B1', 'B2', 'C1', 'C2'].map(t => (
                            <button key={t} onClick={() => setCAPLETarget(t)}
                ...
                <p className="text-[10px] text-stone-300 mt-3 flex items-center gap-1">
                    <AlertTriangle size={10} /> All numbers on this page are practice estimates — not official CAPLE results.
                </p>
                {/* what the chosen target level actually looks like, test by test */}
                <div className="mt-4 bg-stone-900 rounded-2xl p-4 text-white">
                    <p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-2">CAPLE {target} — what your exam looks like</p>
                    <div className="space-y-1.5 text-xs">
                        {([['Reading', CAPLE_LEVEL_FORMATS[target].reading],
                           ['Writing', CAPLE_LEVEL_FORMATS[target].writing],
                           ['Listening', CAPLE_LEVEL_FORMATS[target].listening],
                           ['Speaking', CAPLE_LEVEL_FORMATS[target].speaking]] as [string, string][]).map(([label, fmt]) => (
                    ...
                    <p className="text-[10px] text-white/40 mt-2">{CAPLE_LEVEL_FORMATS[target].note} — the trainers and mock below automatically use {target}-accurate tasks.</p>
                </div>
            </div>
            ...
            <ExamPlanCard
                examName="CAPLE"
                levelLabel={`${CAPLE_OF_LEVEL[getCAPLETarget()] || getCAPLETarget()} target`}
                language="Portuguese"
                plan={getPlan()}
                onSavePlan={savePlan}
            ...
            {/* the CAPLE ladder — ACESSO→DUPLE, no prerequisites */}
            <div className="bg-white rounded-3xl border border-stone-100 p-6">
                <h2 className="font-black text-stone-900 mb-1">The CAPLE ladder</h2>
                <p className="text-xs text-stone-400 mb-3">Six named exams: ACESSO (A1) → CIPLE (A2) → DEPLE (B1) → DIPLE (B2) → DAPLE (C1) → DUPLE (C2). No need to pass lower exams first — register directly for your level.</p>
                <div className="flex gap-1.5 flex-wrap">
                    {(['A1', 'A2', 'B1', 'B2', 'C1', 'C2'] as CapleLevel[]).map(l => (
                        <span key={l} className="text-[11px] font-bold bg-stone-100 text-stone-600 px-2.5 py-1 rounded-xl">{l} → {CAPLE_OF_LEVEL[l]}</span>
                    ))}
                </div>
            </div>
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-6">
                <h2 className="font-black text-stone-900 mb-1">Exam strategy — how CAPLE tries to trick you</h2>
                <p className="text-xs text-stone-400 mb-3">Knowing the traps is worth as many points as knowing the Portuguese.</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {CAPLE_STRATEGY.map(s => (
...
const Curriculum = ({ language }: { language: string }) => {
    ...
    const [level, setLevel] = useState<CapleLevel>('A1');
    const [openTopic, setOpenTopic] = useState<string | null>(null);
    const [lesson, setLesson] = useState<PortugueseLesson | null>(null);
    ...
    const [checkpointFor, setCheckpointFor] = useState<CapleLevel | null>(null);
    ...
    const openLesson = async (topic: { title: string; slug: string; focus: string }) => {
        ...
        const cached = getCachedLesson<PortugueseLesson>(key);
        if (cached) { setLesson(cached); setLessonKey(key); return; }
        setLoading(true);
        try {
            const l = await generatePortugueseLesson(level, topic.title, topic.focus, language as any);
            setLesson(l);
            setLessonKey(key);
            cacheLesson(key, l);
        } catch {
    ...
    const saveAllVocab = () => {
        ...
        lesson.vocabulary.forEach(v => {
            ...
            setSavedVocab(prev => new Set(prev).add(v.pt));
        });
    };

    const topics = CAPLE_SYLLABUS[level];
    const levelPct = (l: CapleLevel) => Math.round(CAPLE_SYLLABUS[l].filter(t => done.includes(`${l}:${t.slug}`)).length / CAPLE_SYLLABUS[l].length * 100);
        ...
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-5">
                <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                    ...
                    {last && (
                        <button onClick={() => {
                            setLevel(last.level as CapleLevel);
                            const topic = CAPLE_SYLLABUS[last.level as CapleLevel]?.find(t => t.slug === last.slug);
            ...
            {locked && !lesson && (checkpointFor === prevLevel ? (
                <CheckpointQuiz
                    ...
                    topics={CAPLE_SYLLABUS[prevLevel].map(t => `${t.title} (${t.focus})`)}
                    onPass={() => {
            ...
            {lesson && (
                <div className="space-y-5">
                    ...
                    <LessonSection title={`Vocabulary (${lesson.vocabulary.length} items)`} icon={<BookOpen size={13} />}>
                        <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                            <p className="text-[11px] text-stone-400">Every item: kanji/kana, rōmaji, reading, example — tap any word for more — tap any word for more.</p>
                                ...
                                <div key={i} className="border border-stone-100 rounded-2xl p-3.5 space-y-1.5 bg-stone-50/50">
                                    <div className="flex items-baseline justify-between gap-2">
                                        <div className="flex items-baseline gap-1.5 flex-wrap">
                                            <InteractiveText text={v.pt} language="Portuguese" className="font-bold text-stone-900" />
                                            {v.gender && (
                                            ...
                                            {v.plural && <span className="text-[10px] font-bold text-stone-400">{v.plural}</span>}
                                        </div>
                                        <button onClick={() => speakText(v.pt, 'Portuguese')} className="text-stone-300 hover:text-emerald-500 shrink-0"><Volume2 size={13} /></button>
                                    </div>
                                    <p className="text-sm text-stone-500">{v.en}</p>
                                    {v.example && <PtEn pt={v.example.pt} en={v.example.en} />}
                            ...
                            <div className="space-y-2">
                                {lesson.pronunciation.map((p, i) => (
                                    <div key={i} className="flex items-center gap-3 bg-stone-50 rounded-xl px-3 py-2">
                                        <span className="font-bold text-stone-800 text-sm">{p.pt}</span>
                                        <span className="text-xs font-mono text-violet-500">/{p.approx}/</span>
                                        <span className="text-xs text-stone-400 flex-1">{p.en}</span>
                                        <button onClick={() => speakText(p.pt, 'Portuguese')} className="text-stone-300 hover:text-emerald-500"><Volume2 size={13} /></button>
                                    </div>
                                ))}
                            </div>
                        </LessonSection>
                            ...
                            <div className="overflow-hidden rounded-2xl border border-stone-100">
                                {lesson.transformations.map((t, i) => (
                                    <div key={i} className={cn('flex items-start gap-3 px-4 py-2.5', i % 2 === 0 ? 'bg-white' : 'bg-stone-50')}>
                                        ...
                                        <div className="flex-1 min-w-0"><PtEn pt={t.pt} en={t.en} /></div>
                                    </div>
                                ))}
                            </div>
                        </LessonSection>
                    ...
                    <LessonSection title="Practice & translation" icon={<CheckCircle2 size={13} />}>
                            ...
                            {lesson.practice.map((ex, i) => {
                                ...
                                return (
                                    <div key={i} className="bg-stone-50 rounded-2xl p-4">
                                        ...
                                        <InteractiveText text={ex.question} language="Portuguese" className="block text-sm font-semibold text-stone-800" />
                                        <div className="flex items-center gap-2 mt-2">
                                            <button onClick={() => setAnswers(prev => ({ ...prev, [`p${i}`]: 'revealed' }))}
                                                className="text-[11px] font-bold text-emerald-600 hover:text-emerald-700">
                                                {picked ? `Answer: ${ex.answer}` : 'Reveal answer'}
                                            </button>
                                            <button onClick={() => speakText(ex.answer, 'Portuguese')} className="text-stone-300 hover:text-emerald-500"><Volume2 size={12} /></button>
                                        </div>
                                    </div>
                                );
                            })}
                            ...
                            {lesson.translationPractice.map((t, i) => {
                                ...
                                return (
                                    <div key={i} className="bg-stone-50 rounded-2xl p-4">
                                        ...
                                        {picked && <div className="mt-1.5"><PtEn pt={t.pt} en="" /></div>}
                                    </div>
                                );
                            })}
                        </div>
                        ...
                        <div className="space-y-3">
                            {lesson.reverseTranslation?.map((t, i) => {
                                ...
                                return (
                                    <div key={i} className="bg-stone-50 rounded-2xl p-4">
                                        <InteractiveText text={t.pt} language="Portuguese" className="block text-sm font-semibold text-stone-800 mb-1.5" />
                    ...
                    {/* culture */}
                    {lesson.culture && (
                        <LessonSection title="Culture — Portugal and the Lusophone world" icon={<Flag size={13} />}>
                    ...
                    <LessonSection title="Mini test" icon={<Trophy size={13} />}>
                        <div className="space-y-4">
                            {lesson.miniTest.map((q, i) => {
                                ...
                                return (
                                    <div key={i}>
                                        <InteractiveText text={q.question} language="Portuguese" className="block text-sm font-bold text-stone-800 mb-2" />
...
// ── Writing trainer ──────────────────────────────────────────────────────────
const WritingTrainer = ({ level, onLevelChange }: { level: CapleLevel; onLevelChange: (l: CapleLevel) => void }) => {
    // Task set is LEVEL-ACCURATE: A1 writes 15–40-word forms, B2 writes 150–180-word essays with sources…
    const tasks = useMemo(() => capleWritingTasksFor(level), [level]);
    ...
    const [feedback, setFeedback] = useState<PortugueseWritingFeedback | null>(null);
    const [error, setError] = useState<string | null>(null);
    ...
    const submit = async () => {
        ...
        try {
            const fb = await evaluatePortugueseWriting(task.label, task.guide, task.minWords, text, level);
            setFeedback(fb);
            const pct = Math.round((fb.score100 / 25) * 100);
            addCAPLEScore({ pct, skill: 'writing', label: `${level} ${task.label}`, score: pct });
        } catch {
            ...
            {feedback && (
                <div className="space-y-4">
                    <div className="bg-white rounded-3xl border border-stone-100 p-6 text-center">
                        ...
                        <p className="text-4xl font-black text-amber-500 my-1">{feedback.score100}<span className="text-lg text-stone-300">/25</span></p>
...
// ── Speaking trainer ─────────────────────────────────────────────────────────
const SpeakingTrainer = ({ level, language, onLevelChange }: { level: CapleLevel; language: string; onLevelChange: (l: CapleLevel) => void }) => {
    // Task set is LEVEL-ACCURATE: A1 has 4 short tasks, B2 adds the graphic monologue, C2 is negotiation…
    const tasks = useMemo(() => capleSpeakingTasksFor(level), [level]);
    const [taskId, setTaskId] = useState(tasks[0].id);
    const task = tasks.find(t => t.id === taskId) ?? tasks[0];
    ...
    const [feedback, setFeedback] = useState<PortugueseSpeakingFeedback | null>(null);
    const [error, setError] = useState<string | null>(null);
    ...
    const scoreSpeaking = async (label: string, combined: string) => {
        ...
        addCAPLEScore({ pct, skill: 'speaking', label, score: pct });
    };

    const finishLiveExam = async (combined: string) => {
        setEvaluatingLive(true); setError(null); setFeedback(null);
        try {
            const fb = await evaluatePortugueseSpeaking(task.label, task.guide, 'Interactive examiner session — multiple questions answered live', combined, level);
            setFeedback(fb);
            await scoreSpeaking(`CAPLE live ${task.label}`, combined);
        } catch {
    ...
    const toggleRecord = () => {
        ...
        rec.promise.then(async (t) => {
            ...
            setFeedback(null);
            try {
                const fb = await evaluatePortugueseSpeaking(task.label, task.guide, task.prompt, t, level);
                setFeedback(fb);
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-6">
                ...
                {mode === 'self' && <InteractiveText text={task.prompt} language="Portuguese" className="block text-sm font-semibold text-stone-800 mb-1" />}
                <p className="text-xs text-stone-400">{task.guide}</p>
...
// ── Progress tab ─────────────────────────────────────────────────────────────
const Progress = ({ scores }: { scores: CAPLEScoreEntry[] }) => {
                ...
                {mocks.slice(0, 5).map((m, i) => (
                    <div key={i} className={cn('flex items-center gap-3 rounded-2xl px-4 py-2.5', i === 0 ? 'bg-stone-900' : 'bg-stone-50')}>
                        ...
                        <span className={cn('text-[9px] font-black px-2 py-0.5 rounded-full', i === 0 ? 'bg-amber-400 text-stone-900' : 'bg-stone-200 text-stone-500')}>
                            {m.passed ? 'SUPERATO' : `weak: ${m.weakest}`}
                        </span>
                    </div>
                ))}
            </div>
    ...
    return (
        <div className="space-y-4">
            ...
            <p className="text-[10px] text-stone-300 text-center flex items-center justify-center gap-1">
                <AlertTriangle size={10} /> Practice estimates only — not official CAPLE results.
            </p>
        </div>
    );
};
...
// ── Portal shell ─────────────────────────────────────────────────────────────
const PortuguesePrepView = () => {
    const { quizSettings, setActiveTab } = useAppStore() as any;
    const language = quizSettings?.targetLanguage || 'Portuguese';
    const [tab, setTab] = useState<JapaneseTab>('overview');
    const [level, setLevel] = useState<CapleLevel>('A2');
    const [scores, setScores] = useState<CAPLEScoreEntry[]>(getCAPLEScores());

    useEffect(() => { setScores(getCAPLEScores()); }, [tab]);

    // Italian-only exam — functional gate, same as TCF for French.
    if (quizSettings?.targetLanguage !== 'Portuguese') {
        return (
            <div className="max-w-md mx-auto w-full py-16 px-6 text-center">
                ...
                <h1 className="text-2xl font-black text-stone-900">The CAPLE portal is Portuguese-only</h1>
                <p className="text-stone-400 text-sm mt-2 leading-relaxed">
                    This portal prepares you for the CAPLE exams (Universidade de Lisboa), so it stays locked
    ...
    const TABS: { id: JapaneseTab; label: string; icon: any }[] = [
    ...
    return (
        <div className="max-w-3xl mx-auto w-full py-8 px-4">
            ...
            <div className="flex items-center justify-between mb-5">
                <div>
                    <h1 className="text-2xl font-black text-stone-900 flex items-center gap-2">
                        <GraduationCap size={24} className="text-emerald-600" /> CAPLE Português
                    </h1>
                    <p className="text-stone-400 text-sm mt-0.5">Exam prep portal — train for the CAPLE exams — in European Portuguese</p>
                </div>
            ...
            {tab === 'vocab' && <PortugueseVocabTrainer level={level} onLevelChange={setLevel} />}
            {tab === 'builder' && <PortugueseSentenceBuilder level={level} onLevelChange={setLevel} />}
            {tab === 'foundations' && (
                <div className="space-y-5">
                    <PortugueseAccentsChart />
                    <PortugueseFoundations />
                </div>
            )}
            {tab === 'cheatsheet' && <PortugueseCheatSheet />}
            {tab === 'mock' && <PortugueseMockExam level={level} onLevelChange={setLevel} />}
            {tab === 'listening' && (
                <PortugueseListeningTrainer level={level} onLevelChange={setLevel} onDone={(pct, label) => {
                    addCAPLEScore({ pct, skill: 'listening', label, score: pct });
                }} />
            )}
            {tab === 'reading' && (
                <PortugueseReadingTrainer level={level} onLevelChange={setLevel} onDone={(pct, label) => {
                    addCAPLEScore({ pct, skill: 'reading', label, score: pct });
                }} />
            )}
...
export default PortuguesePrepView;
