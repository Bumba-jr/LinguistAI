...
export const LevelBar = ({ level, onLevelChange }: { level: JlptLevel; onLevelChange: (l: JlptLevel) => void }) => (
...
export const JapaneseListeningTrainer = ({ level, onLevelChange, onDone }: {
    level: JlptLevel; onLevelChange: (l: JlptLevel) => void;
    onDone: (pct: number, label: string) => void;
}) => {
    const [ex, setEx] = useState<JapaneseListening | null>(null);
    ...
    const start = async () => {
        ...
        try {
            const e = await generateJapaneseListening(level);
    ...
    const playAll = () => {
        ...
        const next = () => {
            ...
            speakText(line.jp, 'Japanese', () => setTimeout(next, 400), rateRef.current);
        };
        next();
    };
        ...
        <div className="space-y-5">
            ...
            {!ex && !loading && (
                <div className="bg-white rounded-3xl border border-stone-100 p-6 space-y-3">
                    <p className="font-black text-stone-900">How JLPT listening works</p>
                            ...
                            {ex.lines.map((l, i) => (
                                <div key={i} className="bg-stone-50 rounded-2xl p-3 space-y-0.5">
                                    ...
                                    <div className="flex items-center justify-between gap-2">
                                        <InteractiveText text={l.jp} language="Japanese" className="block text-sm font-semibold text-stone-900 flex-1" />
                                        <button onClick={() => speakText(l.jp, 'Japanese')} className="text-stone-300 hover:text-indigo-500 shrink-0"><Volume2 size={13} /></button>
                                    </div>
...
export const JapaneseReadingTrainer = ({ level, onLevelChange, onDone }: {
    level: JlptLevel; onLevelChange: (l: JlptLevel) => void;
    onDone: (pct: number, label: string) => void;
}) => {
    const [ex, setEx] = useState<JapaneseReading | null>(null);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [answers, setAnswers] = useState<Record<number, string>>({});
    ...
    const start = async () => {
        ...
        try {
            const e = await generateJapaneseReading(level);
    ...
    return (
        <div className="space-y-5">
            ...
            {!ex && !loading && (
                <div className="bg-white rounded-3xl border border-stone-100 p-6 space-y-3">
                    <p className="font-black text-stone-900">How JLPT reading works</p>
                    <ul className="text-xs text-stone-500 space-y-1.5">
                    ...
                    <div className="bg-white rounded-3xl border border-stone-100 p-6">
                        ...
                        <div className="space-y-3">
                            {ex.paragraphs.map((p, i) => (
                                <div key={i}>
                                    <InteractiveText text={p.jp} language="Japanese" className="block text-sm text-stone-800 leading-relaxed" />
...
export const JapaneseVocabTrainer = ({ level, onLevelChange }: {
    level: JlptLevel; onLevelChange: (l: JlptLevel) => void;
}) => {
    const { addFlashcard, user } = useAppStore() as any;
    const [topicId, setTopicId] = useState(JLPT_VOCAB_TOPICS[0].id);
    ...
    const topic = JLPT_VOCAB_TOPICS.find(t => t.id === topicId)!;

    const generate = async (tid = topicId) => {
        const t = JLPT_VOCAB_TOPICS.find(x => x.id === tid)!;
        ...
        try {
            const r = await generateVocabSet('Japanese', level, t.label, t.hint);
                        ...
                        {words.map((w, i) => (
                            <div key={i} className="border border-stone-100 rounded-2xl p-3.5 bg-stone-50/50 flex items-center gap-3">
                                <div className="min-w-0 flex-1">
                                    {tooltips
                                        ? <InteractiveText text={w.term} language="Japanese" className="font-bold text-lg text-stone-900" />
                                ...
                                <button onClick={() => speakText(w.term, 'Japanese')} className="text-stone-300 hover:text-emerald-500 shrink-0"><Volume2 size={14} /></button>
                            ...
                            <div key={i} className="bg-white rounded-3xl border border-stone-100 p-5">
                                ...
                                {dir === 'en2es' ? (
                                    ...
                                    <div className="flex items-center gap-2 mb-3">
                                        {tooltips
                                            ? <InteractiveText text={q.term} language="Japanese" className="text-xl font-black text-stone-900" />
                                            : <p className="text-xl font-black text-stone-900">{q.term}</p>}
                                        <button onClick={() => speakText(q.term, 'Japanese')} className="text-stone-300 hover:text-emerald-500"><Volume2 size={13} /></button>
                                    </div>
                                )}
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    {options.map((opt, oi) => {
...
export const JapaneseSentenceBuilder = ({ level, onLevelChange }: {
    level: JlptLevel; onLevelChange: (l: JlptLevel) => void;
}) => {
    ...
    const generate = async () => {
        ...
        try {
            const r = await generateSentenceTasks('Japanese', level, 5);
    ...
    const check = async () => {
        ...
        try {
            const r = await checkSentenceAttempts('Japanese', level, tasks.map((t, i) => ({
                            ...
                            <div key={i} className="bg-white rounded-3xl border border-stone-100 p-5 space-y-3">
                                <div className="flex items-start justify-between gap-2">
                                    <div className="min-w-0">
                                        ...
                                        <InteractiveText text={t.base} language="Japanese" className="block text-base font-bold text-stone-900" />
                                        <p className="text-xs text-stone-400">{t.baseEnglish}</p>
                                    </div>
                                    <button onClick={() => speakText(t.base, 'Japanese')} className="text-stone-300 hover:text-emerald-500 shrink-0"><Volume2 size={14} /></button>
                                </div>
                                ...
                                {res && (
                                    <div className={cn('rounded-2xl p-3.5 space-y-1 border',
                                        ...
                                        {res.corrected && res.corrected !== '—' && <InteractiveText text={res.corrected} language="Japanese" className="block text-sm font-bold text-stone-900" />}
