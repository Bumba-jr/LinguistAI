...
import {
    ...
    JLPTScoreEntry, getWeakLog, getMocks,
    cacheLesson, getCachedLesson,
    getCheckpoints, passCheckpoint,
    getPlan, savePlan, clearPlan,
    getJLPTTarget, setJLPTTarget, setLastLesson, getLastLesson,
} from '../../services/japaneseStorage';
import { recordAndTranscribe } from '../../services/speechService';
import { LevelBar, JapaneseListeningTrainer, JapaneseReadingTrainer, JapaneseVocabTrainer, JapaneseSentenceBuilder } from './JapaneseTrainers';
import { JapaneseMockExam } from './JapaneseMockExam';
import { KanaChart, JapaneseFoundations, JapaneseCheatSheet } from './JapaneseFoundations';
import { JLPT_STRATEGY } from '../../services/japaneseFoundation';
...
const LEVELS: JlptLevel[] = ['A1', 'A2', 'B1', 'B2', 'C1', 'C2'];
...
// ── shared UI ────────────────────────────────────────────────────────────────
const JpEn = ({ jp, en, dark = false }: { jp: string; en: string; dark?: boolean }) => (
    <div className="space-y-0.5">
        <InteractiveText text={jp} language="Japanese" dark={dark}
...
const Overview = ({ onGo }: { onGo: (t: JapaneseTab) => void }) => {
    const scores = useMemo(() => getJLPTScores(), []);
    const target = getJLPTTarget();
    ...
    return (
        <div className="space-y-5">
            ...
            <ExamPlanCard
                examName="JLPT"
                levelLabel={`${JLPT_OF_LEVEL[getJLPTTarget()] || getJLPTTarget()} target`}
                language="Japanese"
                plan={getPlan()}
                onSavePlan={savePlan}
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-6">
                <h2 className="font-black text-stone-900 mb-1">Exam strategy — how the JLPT tries to trick you</h2>
                <p className="text-xs text-stone-400 mb-3">Knowing the traps is worth as many points as knowing the Japanese.</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {JLPT_STRATEGY.map(s => (
...
const Curriculum = ({ language }: { language: string }) => {
    ...
    const [lesson, setLesson] = useState<JapaneseLesson | null>(null);
    ...
    const [checkpointFor, setCheckpointFor] = useState<JlptLevel | null>(null);
    ...
    const openLesson = async (topic: { title: string; slug: string; focus: string }) => {
        ...
        const cached = getCachedLesson<JapaneseLesson>(key);
        if (cached) { setLesson(cached); setLessonKey(key); return; }
        setLoading(true);
        try {
            const l = await generateJapaneseLesson(level, topic.title, topic.focus, language as any);
            setLesson(l);
            setLessonKey(key);
            cacheLesson(key, l);
        } catch {
    ...
    const saveAllVocab = () => {
        ...
        lesson.vocabulary.forEach(v => {
            ...
            setSavedVocab(prev => new Set(prev).add(v.jp));
        });
    };

    const topics = JLPT_SYLLABUS[level];
    const levelPct = (l: JlptLevel) => Math.round(JLPT_SYLLABUS[l].filter(t => done.includes(`${l}:${t.slug}`)).length / JLPT_SYLLABUS[l].length * 100);
        ...
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-5">
                <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                    ...
                    {last && (
                        <button onClick={() => {
                            setLevel(last.level as JlptLevel);
                            const topic = JLPT_SYLLABUS[last.level as JlptLevel]?.find(t => t.slug === last.slug);
            ...
            {locked && !lesson && (checkpointFor === prevLevel ? (
                <CheckpointQuiz
                    ...
                    topics={JLPT_SYLLABUS[prevLevel].map(t => `${t.title} (${t.focus})`)}
                    onPass={() => {
            ...
            {lesson && (
                <div className="space-y-5">
                                ...
                                <div key={i} className="border border-stone-100 rounded-2xl p-3.5 space-y-1.5 bg-stone-50/50">
                                    <div className="flex items-baseline justify-between gap-2">
                                        <div className="flex items-baseline gap-1.5 flex-wrap">
                                            <InteractiveText text={v.jp} language="Japanese" className="font-bold text-stone-900" />
                                            <span className="text-xs font-mono text-violet-500">{v.kana}</span>
                                            <span className="text-[10px] font-bold text-stone-400 italic">{v.romaji}</span>
                                        </div>
                                        <button onClick={() => speakText(v.jp, 'Japanese')} className="text-stone-300 hover:text-emerald-500 shrink-0"><Volume2 size={13} /></button>
                                    </div>
                                    <p className="text-sm text-stone-500">{v.en}</p>
                                    {v.example && <JpEn jp={v.example.jp} en={v.example.en} />}
                            ...
                            <div className="space-y-2">
                                {lesson.pronunciation.map((p, i) => (
                                    <div key={i} className="flex items-center gap-3 bg-stone-50 rounded-xl px-3 py-2">
                                        <span className="font-bold text-stone-800 text-sm">{p.jp}</span>
                                        <span className="text-xs font-mono text-violet-500">/{p.approx}/</span>
                                        <span className="text-xs text-stone-400 flex-1">{p.en}</span>
                                        <button onClick={() => speakText(p.jp, 'Japanese')} className="text-stone-300 hover:text-emerald-500"><Volume2 size={13} /></button>
                                    </div>
                                ))}
                            </div>
                        </LessonSection>
                            ...
                            <div className="overflow-hidden rounded-2xl border border-stone-100">
                                {lesson.transformations.map((t, i) => (
                                    <div key={i} className={cn('flex items-start gap-3 px-4 py-2.5', i % 2 === 0 ? 'bg-white' : 'bg-stone-50')}>
                                        ...
                                        <div className="flex-1 min-w-0"><JpEn jp={t.jp} en={t.en} /></div>
                                    </div>
                                ))}
                            </div>
                        </LessonSection>
                    ...
                    <LessonSection title="Practice & translation" icon={<CheckCircle2 size={13} />}>
                            ...
                            {lesson.practice.map((ex, i) => {
                                ...
                                return (
                                    <div key={i} className="bg-stone-50 rounded-2xl p-4">
                                        ...
                                        <InteractiveText text={ex.question} language="Japanese" className="block text-sm font-semibold text-stone-800" />
                                        <div className="flex items-center gap-2 mt-2">
                                            <button onClick={() => setAnswers(prev => ({ ...prev, [`p${i}`]: 'revealed' }))}
                                                className="text-[11px] font-bold text-emerald-600 hover:text-emerald-700">
                                                {picked ? `Answer: ${ex.answer}` : 'Reveal answer'}
                                            </button>
                                            <button onClick={() => speakText(ex.answer, 'Japanese')} className="text-stone-300 hover:text-emerald-500"><Volume2 size={12} /></button>
                                        </div>
                                    </div>
                                );
                            })}
                            ...
                            {lesson.translationPractice.map((t, i) => {
                                ...
                                return (
                                    <div key={i} className="bg-stone-50 rounded-2xl p-4">
                                        ...
                                        {picked && <div className="mt-1.5"><JpEn jp={t.jp} en="" /></div>}
                                    </div>
                                );
                            })}
                        </div>
                        ...
                        <div className="space-y-3">
                            {lesson.reverseTranslation?.map((t, i) => {
                                ...
                                return (
                                    <div key={i} className="bg-stone-50 rounded-2xl p-4">
                                        <InteractiveText text={t.jp} language="Japanese" className="block text-sm font-semibold text-stone-800 mb-1.5" />
                    ...
                    {/* culture */}
                    {lesson.culture && (
                        <LessonSection title="Culture — Japan and everyday life" icon={<Flag size={13} />}>
                    ...
                    <LessonSection title="Mini test" icon={<Trophy size={13} />}>
                        <div className="space-y-4">
                            {lesson.miniTest.map((q, i) => {
                                ...
                                return (
                                    <div key={i}>
                                        <InteractiveText text={q.question} language="Japanese" className="block text-sm font-bold text-stone-800 mb-2" />
...
// ── Writing trainer ──────────────────────────────────────────────────────────
const WritingTrainer = ({ level, onLevelChange }: { level: JlptLevel; onLevelChange: (l: JlptLevel) => void }) => {
    // Task set is LEVEL-ACCURATE: A1 writes 15–40-word forms, B2 writes 150–180-word essays with sources…
    const tasks = useMemo(() => jlptWritingTasksFor(level), [level]);
    ...
    const [feedback, setFeedback] = useState<JapaneseWritingFeedback | null>(null);
    const [error, setError] = useState<string | null>(null);
...
// ── Speaking trainer ─────────────────────────────────────────────────────────
const SpeakingTrainer = ({ level, language, onLevelChange }: { level: JlptLevel; language: string; onLevelChange: (l: JlptLevel) => void }) => {
    // Task set is LEVEL-ACCURATE: A1 has 4 short tasks, B2 adds the graphic monologue, C2 is negotiation…
    const tasks = useMemo(() => jlptSpeakingTasksFor(level), [level]);
    const [taskId, setTaskId] = useState(tasks[0].id);
    const task = tasks.find(t => t.id === taskId) ?? tasks[0];
    ...
    const [feedback, setFeedback] = useState<JapaneseSpeakingFeedback | null>(null);
    const [error, setError] = useState<string | null>(null);
    ...
    const scoreSpeaking = async (label: string, combined: string) => {
        ...
        addJLPTScore({ pct, skill: 'speaking', label, score: pct });
    };

    const finishLiveExam = async (combined: string) => {
        setEvaluatingLive(true); setError(null); setFeedback(null);
        try {
            const fb = await evaluateJapaneseSpeaking(task.label, task.guide, 'Interactive examiner session — multiple questions answered live', combined, level);
            setFeedback(fb);
            await scoreSpeaking(`JLPT live ${task.label}`, combined);
        } catch {
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-stone-100 p-6">
                ...
                {mode === 'self' && <InteractiveText text={task.prompt} language="Japanese" className="block text-sm font-semibold text-stone-800 mb-1" />}
                <p className="text-xs text-stone-400">{task.guide}</p>
...
// ── Progress tab ─────────────────────────────────────────────────────────────
const Progress = ({ scores }: { scores: JLPTScoreEntry[] }) => {
    ...
    return (
        <div className="space-y-4">
            ...
            <p className="text-[10px] text-stone-300 text-center flex items-center justify-center gap-1">
                <AlertTriangle size={10} /> Practice estimates only — not official JLPT results.
            </p>
        </div>
    );
};
...
// ── Portal shell ─────────────────────────────────────────────────────────────
const JapanesePrepView = () => {
    const { quizSettings, setActiveTab } = useAppStore() as any;
    const language = quizSettings?.targetLanguage || 'Japanese';
    const [tab, setTab] = useState<JapaneseTab>('overview');
    const [level, setLevel] = useState<JlptLevel>('A2');
    const [scores, setScores] = useState<JLPTScoreEntry[]>(getJLPTScores());

    useEffect(() => { setScores(getJLPTScores()); }, [tab]);

    // Italian-only exam — functional gate, same as TCF for French.
    if (quizSettings?.targetLanguage !== 'Japanese') {
        return (
            <div className="max-w-md mx-auto w-full py-16 px-6 text-center">
                ...
                <h1 className="text-2xl font-black text-stone-900">The JLPT portal is Japanese-only</h1>
                <p className="text-stone-400 text-sm mt-2 leading-relaxed">
                    This portal prepares you for the Japanese-Language Proficiency Test, so it stays locked
    ...
    return (
        <div className="max-w-3xl mx-auto w-full py-8 px-4">
            ...
            <div className="flex items-center justify-between mb-5">
                <div>
                    <h1 className="text-2xl font-black text-stone-900 flex items-center gap-2">
                        <GraduationCap size={24} className="text-emerald-600" /> JLPT 日本語
                    </h1>
                    <p className="text-stone-400 text-sm mt-0.5">Exam prep portal — train for the JLPT — and for real Japanese beyond it</p>
                </div>
            ...
            {tab === 'vocab' && <JapaneseVocabTrainer level={level} onLevelChange={setLevel} />}
            {tab === 'builder' && <JapaneseSentenceBuilder level={level} onLevelChange={setLevel} />}
            {tab === 'foundations' && (
                <div className="space-y-5">
                    <KanaChart />
                    <JapaneseFoundations />
            ...
            {tab === 'cheatsheet' && <JapaneseCheatSheet />}
            {tab === 'mock' && <JapaneseMockExam level={level} onLevelChange={setLevel} />}
            {tab === 'listening' && (
                <JapaneseListeningTrainer level={level} onLevelChange={setLevel} onDone={(pct, label) => {
                    addJLPTScore({ pct, skill: 'listening', label, score: pct });
                }} />
            )}
            {tab === 'reading' && (
                <JapaneseReadingTrainer level={level} onLevelChange={setLevel} onDone={(pct, label) => {
                    addJLPTScore({ pct, skill: 'reading', label, score: pct });
                }} />
            )}
...
export default JapanesePrepView;
